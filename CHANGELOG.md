...
### City scalars are checked against physics, and WolframAlpha is rejected on licence (2026-09-08)
- **`area_km2` and `elevation_m` had no validator branch anywhere in the pipeline**, population was checked only for `isFinite && >= 0`, and no section of `check-pipeline-health.mjs` read the validate stage at all. Measured on prod: **181** cities with an impossible area, 1 with an impossible elevation, **4 more populous than their own country**, 35 at an impossible density. El Reno held an area of 8,300,000,226 km²; City of Hamilton 1,138,110,000 km², about seven times the land area of Earth; Maui an elevation of 10,023 m, above Everest; Norfolk a population of 343,000,000, more than the entire United States.
- **The producer was a unit bug, and Calgary names it.** It stored 825,290,000 for a city of 825.29 km² — square metres under the km² label. `parseCityFacts` read a Wikidata quantity's `.amount` and ignored its `.unit`; sampling 15 cities on the live API found P2046 stated in `Q25343` (square metre) and P2044 in `Q3710` (foot). It now converts through an explicit table or **writes nothing**: an unrecognised unit is a measurement in an unknown scale, and the bare amount is exactly the defect. Every unit QID was resolved against the live label API rather than recalled — `Q2489629`, reached for as "are", is a Dutch cyclist.
- **Nothing overwrote these values; fill-if-empty protected them.** For **234 cities** the engine had already fetched the correct population, recorded it in `field_provenance.population.candidates`, found the column non-empty and correctly declined — Paris still carries Wikidata's 2,103,778 in provenance beside 12,000,000 in the column. A wrong *seed* value is permanent by design. This is why no new external source would have fixed it: the right answer was already fetched and already stored.
- **The repair retracts to NULL and never recomputes.** 58% of the impossible areas are a clean factor of 10⁶, which makes "just divide" tempting and wrong — the other 42% would acquire a *plausible* wrong number, which is never looked at again (El Reno becomes 8,300 km² for a city of ~44). A NULL is also self-healing: the unit fix is upstream and the backfill is fill-if-empty, so an emptied column refills correctly on that city's next visit. Retracted values are preserved under `field_provenance.<col>.retracted`.
- **The guard sits on the producer that actually wrote them, which is not the validator.** `validateCityNormalized` has exactly one caller, `pipeline-validate`, and it only ever reads `ingestion_staging`; all 181 rows came from `city-factual-backfill`, which `UPDATE`s `cities` directly. `plausibleCityScalar` (`_shared/city-scalar-bounds.ts`, one definition imported by both paths) is now called there before `addCandidate`, so an impossible value never becomes provenance either, and refusals are counted per run rather than dropped.
- **The bounds are warnings, never errors.** `pipeline-validate` turns any error into `rejected`, and the human-approval trigger promotes only `pending`/`needs_review` — a hard rejection is the one state an admin can never override. As errors these would have discarded a whole staged city, coordinates and legal payload included, because one number was wrong.
- **Density is reported, never retracted, and deliberately not ratcheted** — 33 rows survive by design. A density defect does not say *which* column is wrong (Paris holds a metro population over a commune area). The repair nulled 181 areas and the density arm requires a positive area, so those rows left the population by construction and re-enter it as they refill: the number is designed to RISE, and a shrinking baseline would red the build with no legal response.
- **New gate:** `city_scalar_defects()` + section 11b of `check-pipeline-health.mjs` — zero tolerance on the three single-column bounds, advisory on density, an **absent-key check** (a renamed key would arrive as `undefined` and report a clean corpus having measured nothing), and a **404-vs-5xx split** so a broken probe fails rather than warns. `retracted_pending_refill` excludes `qid_conflict` rows, which the backfill skips and which therefore can never refill.
- **The repair does not set `needs_attention`** — `run_city_trust_recompute` subtracts 0.15 for it (a trust penalty for a field completeness never reads) and `approve_city_review` clears it unconditionally, so it is both disproportionate and not durable. `field_provenance.<col>.retracted` is the record.
- **WolframAlpha is rejected, on licence, and this was the third proposal.** Its API terms forbid the one operation every ingest path performs: *"Your API Client is prohibited from caching Wolfram|Alpha content"*, and on termination *"delete all API Results in Your possession or control."* The free tier is 2,000 calls/month **non-commercial only**; commercial display requires a live embed with attribution. It also fails the rule against resolving an entity by name alone — free text in, `<assumption>` out. Recorded in `docs/architecture/open-data-integration.md` §3.4 so it is not proposed a fourth time. Note `countries.wolfram_enriched_at` remains load-bearing as the round-robin cursor in `pipeline-enrich-country-stats` and must not be cleaned up.
