...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const countryIds = [...new Set(places.map((p: any) => p.country_id).filter(Boolean))];
    let countries: any[] = [];
    ...
    return new Response(JSON.stringify({ error: 'Failed to generate trip PDF', details: String(err) }), {
      status: 500,
      headers: { ...cors, 'Content-Type': 'application/json' },
    });
  }
