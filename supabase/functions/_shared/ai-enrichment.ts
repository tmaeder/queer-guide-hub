...
// ---------------------------------------------------------------------------
// Venue grounded enrichment (from the venue's OWN web page)
// ---------------------------------------------------------------------------

export interface VenueMoatEnrichment {
  description?: string
  tags?: string[]                       // lowercase-hyphenated
  category?: string                     // bar|club|restaurant|cafe|community-center|sauna|hotel|shop|gallery|other
  accessibility_attributes?: string[]
  target_groups?: string[]              // trans, women, bears, qtbipoc, ...
  amenities?: string[]
  lgbtq_relevance_score?: number
  confidence?: number                   // 0.0-1.0 — how well the page supported the extraction
}

const VENUE_MOAT_KEYS = ['description', 'tags', 'category', 'accessibility_attributes',
  'target_groups', 'amenities', 'lgbtq_relevance_score', 'confidence']

const VENUE_MOAT_SYSTEM_PROMPT = `${BASE_CONTEXT}

You extract structured fields for an LGBTQ+ venue directory from a venue's OWN web page text. This is GROUNDED EXTRACTION, not creative writing.

Hard rules:
- Use ONLY facts present in the provided page text. If a field is not stated, set it to null or omit it. NEVER invent details, policies, or an LGBTQ+ identity the page does not support.
- description: a factual 2-3 sentence summary built only from page facts.
- tags: up to 5 lowercase-hyphenated descriptors actually supported by the page.
- category: one of bar, club, restaurant, cafe, community-center, sauna, hotel, shop, gallery, other.
- accessibility_attributes: lowercase-hyphenated flags mentioned (wheelchair-accessible, step-free, gender-neutral-restrooms, ...).
- target_groups: who it is for, if stated (trans, women, bears, qtbipoc, youth, ...).
- amenities: concrete amenities mentioned (outdoor-seating, dance-floor, darkroom, wifi, ...).
- lgbtq_relevance_score: 0.0-1.0 how clearly the page shows this is an LGBTQ+ venue (0 if no explicit LGBTQ+ signal).
- confidence: 0.0-1.0 how well the page text supported this extraction (low if the page was thin/irrelevant/a parked domain).

Respond ONLY with valid JSON. No markdown code blocks.`

/**
 * Extract venue fields from a venue's source page. Grounded in pageText; the caller
 * circuit-breaks and applies hybrid-by-confidence (fill EMPTY fields only).
 */
export async function researchEnrichVenueFromPage(
  supabase: SupabaseClient,
  input: { name: string; city?: string; country?: string; category?: string; existingDescription?: string; pageText: string },
): Promise<VenueMoatEnrichment | null> {
  if (!(await isOpenAIAvailable(supabase))) return null
  const page = (input.pageText || '').slice(0, 6000)
  if (page.trim().length < 80) return null   // nothing to ground on — skip the LLM call

  const userPrompt = `Venue: ${ud(input.name)}
City: ${ud(input.city || 'N/A')} | Country: ${ud(input.country || 'N/A')} | Current category: ${ud(input.category || 'N/A')}
${input.existingDescription ? `Existing description: ${ud(input.existingDescription.slice(0, 300))}` : ''}
...
Respond with JSON using these keys (null where unknown):
{"description":"...","tags":[...],"category":"...","accessibility_attributes":[...],"target_groups":[...],"amenities":[...],"lgbtq_relevance_score":0.0,"confidence":0.0}`

  try {
    const result = await chatCompletion(supabase, {
      messages: [
        { role: 'system', content: VENUE_MOAT_SYSTEM_PROMPT },
        { role: 'user', content: userPrompt },
      ],
      temperature: 0.2,
      max_tokens: 600,
    ...
    return parseAIResponse<VenueMoatEnrichment>(result.content, VENUE_MOAT_KEYS)
  } catch (err) {
    console.error('Venue moat enrichment failed:', (err as Error).message)
    return null
  }
}
