...
//
// Wraps public.recommend_venue_guides() SQL scorer. Mirror of
// marketplace-recommend but for venue guides.
//
// Signed-in users get the full personalization (city + interests +
// category affinity + freshness + featured + continue-reading).
// Anon callers get freshness + featured only.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('recommend_venue_guides', {
      p_user_id: userId,
      p_limit: limit,
    })
    ...
    return jsonResponse(
      { guides, count: guides.length, personalized: userId !== null },
      200,
      req,
    )
  } catch (err) {
