// Synonym validation + projection into Meilisearch settings.synonyms.
...
export function validateSynonym(raw: Partial<SynonymInput>): ValidationResult {
  ...
  if (replacements.length === 0) errors.push('replacements must contain at least one non-empty string')
...
/**
 * Project a list of active synonym rows into Meilisearch's per-index synonym
 * map. Two-way synonyms become entries for every term pointing to terms +
 * replacements; one-way become entries only for terms pointing to replacements.
 */
