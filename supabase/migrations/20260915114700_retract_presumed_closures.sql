...
CREATE OR REPLACE FUNCTION public._existence_apply_archive(
  ...
  -- Defaults MUST be repeated. The live function declares them, and
  -- CREATE OR REPLACE cannot remove a parameter default (42P13) — omitting
  -- them aborts the whole migration.
...
CREATE OR REPLACE FUNCTION public._existence_apply_reopen(
  -- p_actor's DEFAULT must be repeated for the same reason as
  -- _existence_apply_archive above: CREATE OR REPLACE cannot drop it (42P13).
  p_entity_type text, p_entity_id uuid, p_actor uuid DEFAULT NULL::uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_temp'
