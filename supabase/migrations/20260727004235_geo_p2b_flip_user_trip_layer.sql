-- P2 redo B4: user + trip layer — trip_places computed rels + favorites bonus rels
-- (favorites have NO FK at all; their embeds in recommendation-engine were already
-- broken — the rels fix them) + FK flips.
set local lock_timeout = '10s';

create or replace function public.cities(tp public.trip_places)
returns setof public.cities language sql stable rows 1
as $$ select * from public.cities where id = tp.city_id $$;
create or replace function public.countries(tp public.trip_places)
returns setof public.countries language sql stable rows 1
as $$ select * from public.countries where id = tp.country_id $$;

create or replace function public.cities(f public.city_favorites)
returns setof public.cities language sql stable rows 1
as $$ select * from public.cities where id = f.city_id $$;
create or replace function public.countries(f public.country_favorites)
returns setof public.countries language sql stable rows 1
as $$ select * from public.countries where id = f.country_id $$;
...
alter table public.trip_destinations add constraint trip_destinations_village_id_fkey
  foreign key (village_id) references public.geo_village_profiles(place_id) on delete set null;
...
alter table public.trip_geo_review_queue add constraint trip_geo_review_queue_resolved_city_id_fkey
  foreign key (resolved_city_id) references public.geo_city_profiles(place_id);
...
alter table public.trip_geo_review_queue add constraint trip_geo_review_queue_resolved_country_id_fkey
  foreign key (resolved_country_id) references public.geo_country_profiles(place_id);
...
alter table public.trips add constraint trips_primary_city_id_fkey
  foreign key (primary_city_id) references public.geo_city_profiles(place_id) on delete set null;
...
alter table public.trips add constraint trips_primary_country_id_fkey
  foreign key (primary_country_id) references public.geo_country_profiles(place_id) on delete set null;
...
alter table public.user_travel_preferences add constraint user_travel_preferences_home_city_id_fkey
  foreign key (home_city_id) references public.geo_city_profiles(place_id) on delete set null;
...
alter table public.user_travel_preferences add constraint user_travel_preferences_home_country_id_fkey
  foreign key (home_country_id) references public.geo_country_profiles(place_id) on delete set null;
...
alter table public.intimate_profiles add constraint intimate_profiles_discovery_city_id_fkey
  foreign key (discovery_city_id) references public.geo_city_profiles(place_id) on delete set null;
