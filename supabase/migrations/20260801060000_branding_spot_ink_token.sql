-- Riso spot ink: make `spot` / `spot-foreground` overridable at runtime.
--
-- The new brand accent has to be tunable from /admin/design without a deploy,
-- which means branding_validate's key whitelist must accept it — otherwise
-- saving a draft that touches it raises "token spot is not an overridable
-- color token".
--
-- CREATE OR REPLACE requires the whole function body, so this is the
-- 20260724160000 definition verbatim with exactly two keys appended to
...
DECLARE
  v_color_keys text[] := ARRAY[
    ...
    'success','success-foreground','spot','spot-foreground','border','input','input-bg','ring',
