-- §8.6 target-group facet + filter. target_groups (women / youth / families /
-- lesbian / non-binary / leather / …) is populated on venues+events; expose it as
-- a filterable facet. (accessibility arrays are empty in the corpus today, so it's
-- deferred until that data lands.) Supersets the current live search_hybrid
-- (news recency) and search_facets (price/date histograms); pg-path only.

-- search_hybrid + target_groups filter (?| = any-of match on the jsonb array)
...
-- search_facets + target_groups facet (unnest the jsonb array)
