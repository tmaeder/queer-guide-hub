-- Reconcile tag_hygiene_stats() with the live database.
--
-- The live function returns `indexable_marketplace_facet` and NO migration in
-- this repo defines it. It was added out-of-band, so the repo stopped being the
-- source of truth for this function and two guards began contradicting each
-- other:
--
--   * scripts/check-tag-hygiene.mjs calls the LIVE function, saw a metric with
--     no baseline entry, and hard-failed. "Critical data-quality gates" has
--     been red on every open PR since -- #3363, #3367, #3371, #3372, #3381 --
--     none of which touch tags.
--   * src/lib/__tests__/tagHygienePanelMetrics.test.ts compares the baseline
--     against the REPO's newest definition, so simply adding the baseline key
--     fixed the first guard and broke the second. Neither could be satisfied
--     alone; the drift itself had to go.
--
-- This redefinition is the repo's own most recent definition
-- (20261211120300_tag_hygiene_language_sentinels.sql) with the missing metric
-- block inserted, NOT a verbatim dump of the live body -- the live body had
-- been stripped of ~4KB of comments, and in this codebase those comments are
-- the documentation of why each sentinel exists.
--
-- Equivalence was verified rather than assumed. Normalising both sides
-- (comments removed, whitespace collapsed, case folded) the two agree
-- byte-for-byte over the first 4,950 of 5,389 characters, and the remaining
-- difference is confined to a `non_latin_name` character class that Postgres
-- lower() and Python str.lower() fold differently, plus a trailing semicolon.
-- The top-level key sets are identical: 26 each, including the new metric.
--
-- `create or replace` preserves existing grants, so none are restated.
...
begin
  ...
  select jsonb_build_object(
    ...
    -- glossary page. A facet is a query control, not a concept: it has no
    -- definition to write, so the page is thin by construction and can only be
    -- fixed by deindexing. Zero-invariant.
