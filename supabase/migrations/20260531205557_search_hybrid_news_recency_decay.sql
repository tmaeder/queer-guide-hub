-- §14.3 per-type temporal decay: give news a recency boost (events already get an
-- imminence boost). news.start_date holds published_at; fresher articles rank
-- higher, decaying over ~30 days. Additive to the scored CTE; everything else in
-- search_hybrid is unchanged. Affects only the pg/shadow path (live default is meili).
