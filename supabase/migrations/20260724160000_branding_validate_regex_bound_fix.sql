-- Fix: branding_validate used regex bounds {1,300}, but PostgreSQL's regex
-- engine caps repetition bounds at 255 — a {1,n>255} bound throws
-- "invalid regular expression: invalid repetition count(s)" the moment the
-- regex is COMPILED (i.e. whenever a URL-bearing field is validated).
--
-- This was latent since the control center shipped: no override doc had yet
-- set og_image_url / org_logo_url / org_sameas / email.logo_url / a font URL,
-- so the offending regexes were never compiled. The fonts work surfaced it.
--
-- Fix: every {1,300} → {1,255}. 255-char URLs are ample; nothing else changes.
-- CREATE OR REPLACE is idempotent (already applied live via execute_sql).
