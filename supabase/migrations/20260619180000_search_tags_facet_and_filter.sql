-- Integrate tags into search: add a `tags` facet to search_facets and a `tags`
-- any-of filter to search_hybrid, mirroring the existing `target_groups` pattern
-- exactly. Tags already flow into search_documents.facets->'tags' (venues, events,
-- community_groups) and search_hybrid already RETURNS facets->'tags' per hit — this
-- only adds the facet aggregation + the filter clause so the UI can drill into tags.
-- No table writes, so no search-sync trigger storm / disk risk.
--
-- Bodies are copied from the LIVE definitions (search_hybrid reads embeddings from
...
-- ── search_hybrid: add the `tags` any-of filter clause (output already returns
--    facets->'tags'). Body copied from LIVE (sibling search_embeddings + image
--    asset lateral join). Signature unchanged, so assert_search_hybrid_contract()
--    holds.
