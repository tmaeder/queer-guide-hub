-- Move compute_visibility_score axis weights from hardcoded constants into
-- search_settings_versions. The function previously declared seven
-- `constant numeric` weights at the top of its declare block — operators
-- couldn't tune relevance without a code deploy. This migration:
--
--   1. Seeds the initial active weights row in search_settings_versions
--      (index_name='visibility_score', version=1, channel='active').
--   2. Replaces compute_visibility_score with a version that loads weights
--      from the latest active row at function entry. If no active row exists
--      (operator deleted it?), the function falls back to the same defaults
--      so scoring keeps working.
--
-- Mirrors the pattern doc/search-intelligence/02-unified-model.md describes
-- for moving relevance config into versioned DB rows. Body of the function
-- is otherwise identical to the v2 migration (20260428160000); only the
-- weight handling at the top of the declare/begin blocks differs.

-- ── Seed initial weights ────────────────────────────────────────────────────

insert into public.search_settings_versions
  (index_name, version, channel, settings, comment)
values (
  'visibility_score',
  1,
  'active',
  jsonb_build_object(
    'axis_weights', jsonb_build_object(
      'tags',     0.20,
      'geo',      0.15,
      'images',   0.15,
      'dates',    0.10,
  ...
  'Initial weights migrated from hardcoded constants in compute_visibility_score v2'
)
on conflict (index_name, version) do nothing;

-- ── Rewrite compute_visibility_score ────────────────────────────────────────
...
declare
  ...
  -- Weights (loaded from search_settings_versions at function entry; the
  -- defaults below match the seed row in this migration and act as a fallback
  -- if an operator deletes the row.)
  w_tag   numeric := 0.20;
  w_geo   numeric := 0.15;
  w_image numeric := 0.15;
  w_date  numeric := 0.10;
  w_text  numeric := 0.20;
  w_syn   numeric := 0.10;
  w_query numeric := 0.10;
...
begin
  -- Load the latest active axis weights. coalesce keeps the declared defaults
  -- on each axis if the JSON value is null/missing — the function never sees
  -- a null weight.
  select settings->'axis_weights' into v_axis_weights
    from public.search_settings_versions
   where index_name = 'visibility_score'
     and channel    = 'active'
...
comment on function public.compute_visibility_score(text, uuid) is
  'Compute the Search Visibility Score for an entity. Reads axis weights '
  'from search_settings_versions (index_name=''visibility_score'', '
  'channel=''active'', latest version) at function entry. Returns the '
  'JSONB shape documented in docs/search-intelligence/02-unified-model.md.';
