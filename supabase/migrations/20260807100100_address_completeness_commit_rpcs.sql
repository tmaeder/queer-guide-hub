-- ============================================================================
-- Address completeness (2/3): stop the ingestion pipeline discarding the fields
-- ----------------------------------------------------------------------------
-- The backfill in 3/3 is pointless if ingestion keeps re-creating the gap.
-- Three defects, all confirmed by reading the LIVE pg_proc source (the
-- migration files for both RPCs had drifted from what is deployed):
--
--   1. commit_venue_staging_item DECLAREs v_country_id, resolves it, and then
--      never puts it in the INSERT column list. Every venue committed by the
--      pipeline landed with country_id = NULL.
--   2. That same country lookup was nested INSIDE `IF v_loc->>'city' IS NOT
--      NULL`, so a venue with a country but no city text never resolved a
--      country at all.
--   3. Neither RPC wrote state or postal_code — the columns simply were not in
--      the INSERT/UPDATE lists — and commit_event_staging_item never set
--      city_id or country_id either (the string "country_id" did not appear
--      anywhere in its body).
--
-- Note: an earlier analysis claimed commit_event_staging_item stamps
-- `coalesce(v_country, 'US')`, silently marking country-less events as United
-- States. That is FALSE in the deployed function — it reads
-- `coalesce(v_country, '')`. It came from a stale migration file. Left as is.
-- ============================================================================
...
-- search_path is 'public', 'pg_temp' on the deployed function (the venue RPC
-- uses 'public', 'extensions'). Preserved verbatim — this body calls
-- extensions.digest() with an explicit schema qualifier, so it does not need
-- extensions on the path, and widening it would be an unrelated behaviour change.
