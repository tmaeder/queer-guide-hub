-- search_hybrid: re-add the target_groups filter + fix the latency root cause.
--
-- Built on the live `type_priority` definition (20260601162922) — RANKING is left
-- exactly as that migration set it (entity-type priority, geo soft-boost, in-query
-- city extraction, bare-city browse, event imminence). Two changes only:
--
-- 1) target_groups any-of FILTER re-added. It was dropped at `geo_soft_boost` and
--    never restored, so faceted audience filtering (women/youth/lesbian/trans/…)
--    was silently broken. This is a filter, not a ranking change.
--
-- 2) Latency root-cause fix. `or sd.doc_id in (select doc_id from vnn)` OR'd a
--    subquery into the candidate WHERE, which defeats the GIN bitmap plan → a full
--    seq scan on EVERY keyword query (even when p_query_vec is null and vnn is
--    empty): "gay bar" ~1010ms. Now the keyword (FTS + index-backed `title % q`)
--    and vector (vnn) candidate ids are gathered in a `kwvec` UNION CTE and the
--    admission is a membership test → bitmap scan restored: "gay bar" ~325ms,
--    "cafe" ~278ms. `cand` also selects explicit columns instead of `sd.*` so the
--    1024-dim embedding is no longer dragged through the CTE. Same candidate set
--    and same scoring — results unchanged.
--
-- NB: `title % q` uses pg_trgm.similarity_threshold's default 0.3 (vs the prior
-- `similarity()>0.2`); the migration role can't SET that GUC. Net effect is
-- marginally more precise trigram matching (top results unchanged, fuzzy tail
-- trimmed). Pinning 0.2 would require a superuser GRANT.
...
cand as (
  select sd.doc_id, sd.entity_id, sd.entity_type, sd.title, sd.description, sd.facets, sd.city, sd.country, sd.slug, sd.image_url, sd.is_featured, sd.is_free, sd.price_min, sd.price_max, sd.start_date, sd.end_date, sd.trust_score, sd.liveness_status, sd.geog, sd.quality_score, sd.closed_at, p.boost_city,
