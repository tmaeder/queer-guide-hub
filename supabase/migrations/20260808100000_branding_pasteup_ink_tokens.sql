-- PASTE-UP inks: make the 2nd and 3rd drums overridable at runtime.
--
-- The rebrand adds --ink-blue / --ink-over (plus their paired foregrounds) as
-- brand ink alongside the existing --spot. They have to be tunable from
-- /admin/design without a deploy, which means branding_validate's key
-- whitelist must accept them — the function RAISES on an unknown key rather
-- than dropping it, so without this a draft touching an ink fails with
-- "token ink-blue is not an overridable color token".
--
-- There is deliberately NO `ink-pink` key: that ink is a @theme alias of
-- `spot` in src/index.css, so overriding `spot` moves both and the two can
-- never drift apart.
--
-- CREATE OR REPLACE requires the whole function body, so this is the
-- 20260801060000 definition verbatim with exactly four keys appended to
-- v_color_keys. Diff it against that file to confirm nothing else moved.
--
