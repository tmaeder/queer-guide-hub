-- Narrow branding_validate's colour allowlist: the PASTE-UP ink aliases are gone.
--
-- `--spot`, `--spot-foreground`, `--ink-blue(-foreground)` and
-- `--ink-over(-foreground)` were the PASTE-UP names for three colours the
-- subway-map system already owns as `--track-pink` / `--track-blue` /
-- `--track-green`. They were kept as aliases so existing call sites kept
-- rendering; those call sites are now gone, and so are the tokens
-- (src/index.css, tokenCatalog.ts, functions/_lib/branding.ts).
--
-- Left in this allowlist they would let /admin/design publish an override for
-- a token nothing reads: the edge middleware would inject `--spot: …` into
-- <head> and no rule would consume it. That is a silent no-op control, which
-- is worse than an absent one.
--
-- Checked before writing this: `site_branding.published`, `site_branding.draft`
-- and every row of `site_branding_versions` carry ZERO overrides using any of
-- the six keys, so no stored document becomes unreplayable. That mattered —
-- `branding_revert` replays a stored doc back through this same validator, and
-- this function RAISEs on an unknown key.
--
-- The body below is the live definition with only those six entries removed;
-- verified byte-identical to `pg_get_functiondef` output otherwise (md5
-- 9087d79a1720b2f2a84c41468b584f36, 11346 bytes). Restated in full rather than
-- patched by string surgery, which is the lesson of 20260806140000 /
-- 20260809100000: a surgically-patched live function leaves the repo copy
-- wrong.
...
DECLARE
  ...
  v_lh_keys text[] := ARRAY[
    ...
    'text-title--line-height','text-headline--line-height',
  ...
  v_url_re      text := '^(https://|/)[^\s"''<>]{1,255}$';
  v_hex_re      text := '^#[0-9a-fA-F]{6}$';
  v_font_url_re text := '^(https://xqeacpakadqfxjxjcewc\.supabase\.co/storage/v1/object/public/brand/|/fonts/)[^\s"''<>]{1,255}\.woff2$';
  ...
  IF p_doc ? 'fonts' THEN
    ...
    FOR v_slot IN SELECT jsonb_object_keys(p_doc->'fonts') LOOP
      ...
      IF (v_slot_obj->>'family') IS NULL OR (v_slot_obj->>'family') !~ v_family_re THEN
        RAISE EXCEPTION 'fonts.%.family must match letters/digits/space/_/- (1-60)', v_slot;
      END IF;
    ...
    FOR v_key, v_val IN SELECT * FROM jsonb_each(p_doc->'meta') LOOP
      ...
      IF v_key = 'org_sameas' THEN
        ...
        FOR v_item IN SELECT * FROM jsonb_array_elements(v_val) LOOP
          IF jsonb_typeof(v_item) <> 'string' OR (v_item #>> '{}') !~ '^https://[^\s"''<>]{1,255}$' THEN
