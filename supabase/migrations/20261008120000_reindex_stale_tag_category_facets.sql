-- RENUMBERED from 20261007140000, content unchanged.
--
-- That version was shared with `prevention_bathroom_plural_twin`, which merged
-- from a different PR and applied first. `db push` matches history by version
-- alone, so it applied that one, then hit
-- `duplicate key value violates unique constraint "schema_migrations_pkey"`
-- on this file and ABORTED — abandoning every later migration. Prod sat at
-- 20261007140000 with the whole backlog behind it, and the deploy run went red
-- with "MERGED MIGRATIONS DID NOT APPLY".
--
-- (#3126 moved the drgay wave out of the same three-way collision; this is the
-- last member of it.)
--
-- Renumbered ABOVE the repo head rather than into a free slot below it: a
-- version not already in remote history must sort above the newest existing
-- one, or `check-migration-versions` rejects it and `db push` would abort on it
-- anyway.
--
