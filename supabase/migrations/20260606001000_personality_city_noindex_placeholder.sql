-- Prevention: auto-created personality-birthplace placeholder cities are born
-- non-indexable, so they never leak into the explore map, /cities directory,
-- sitemap, or search (the root cause behind the "Lucerne next to Berlin" bug).
-- Only change vs 20260510150000 is `seo_indexable = false` on the INSERT.
...
BEGIN
  ...
  THEN
    -- Insert with ON CONFLICT to handle races. seo_indexable = false: this is a
    -- placeholder stub, kept out of public surfaces until promoted to a real city.
    INSERT INTO cities (name, country_id, slug, data_source, seo_indexable)
    VALUES (_birth_clean, _country_id, 'tmp-' || gen_random_uuid(), 'personality-birth-place', false)
