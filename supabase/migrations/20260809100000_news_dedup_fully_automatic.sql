-- News dedup becomes fully automatic: it auto-merges or it drops. It never
-- queues a pair for a human, in any mode.
--
-- WHY
--   The nightly sweep queued 406 news pairs in one week (153 on 07-26, 124 on
--   08-01). On 2026-08-01 all 406 had to be bulk-rejected in a single pass --
--   not one was a duplicate. That is the entire history of the news branch:
--   it has never produced a true positive and has only ever produced work.
--
--   20260806140000 stopped the bleeding by requiring `a.url is not distinct
--   from b.url`, but over-corrected: news_articles.fingerprint is UNIQUE, so a
--   same-URL re-ingest can never reach the sweep in the first place. The rule
--   became unreachable and news merging died. 172 same-source/same-day/
--   same-title clusters (194 extra rows) are sitting live with nothing that
--   will ever look at them again.
--
-- WHAT CHANGES
--   1. The news gate now requires same source_id + same published day + same
--      despaced title AND a byte-identical body (or an identical
--      canonical_url). Measured on the live corpus: 17 pairs out of the 218
--      same-source/day/title pairs. Small by design -- that IS the whole
--      true-duplicate population. The other 201 are distinct articles that
--      merely share a headline, and 4 of them are podcast episodes that share
--      an episode title. See the news branch below for the two signals that
--      were measured and rejected.
--   2. News can never enter dedup_review_queue. Two guards, because the queue
--      branch is reachable whenever mode <> 'full' regardless of is_auto.
--   3. triage_src_dedup_review excludes news, so it cannot appear in the admin
--      inbox or its badge (adminQueues resolves quality_duplicates through
--      triage_sources.count_key, which reads this view -- no separate count).
--
-- DO NOT key this on news_articles.content_hash. That column is currently
-- poison: 7,501 rows from one source all carry hash d79f1889... across 7,387
-- DISTINCT titles and content lengths 0 -> 153 KB, and 1,129 rows carry
-- e3b0c442..., the SHA-256 of the empty string. Keying on it would merge 8,943
-- distinct articles. 20260809120000 repairs it.
--
-- This file is a FULL CREATE OR REPLACE, transcribed from the live prod
-- definition (pg_get_functiondef), not another string-surgery patch.
-- 20260806140000 rewrote the live function in place, so the repo copy in
-- 20260725201000 has been wrong ever since -- any CREATE OR REPLACE pasted
-- from that file would have silently reverted both the news URL guard and the
-- marketplace token-arm deletion. Both of those fixes are carried forward
-- here (the marketplace branch keeps only the same_merchant_key arm), and the
-- repo is now the source of truth again.
...
-- Keep news out of the admin inbox and its badge. Same column list and order as
-- 20260725202000 -- CREATE OR REPLACE VIEW cannot change either, and the grants
-- carry over.
CREATE OR REPLACE VIEW public.triage_src_dedup_review AS
  SELECT id,
    'dedup-review'::text AS queue_type,
    entity_type AS content_type,
    (COALESCE((cluster -> 'keep'::text) ->> 'title'::text, '?'::text) || ' ⇄ '::text)
      || COALESCE((cluster -> 'drop'::text) ->> 'title'::text, '?'::text) AS title,
    reason AS subtitle,
    status,
    confidence AS confidence_score,
    created_at,
    source,
    keep_id AS entity_id,
    entity_type AS entity_table,
    false AS has_diff,
    NULL::uuid AS reporter_id,
    cluster AS meta,
    NULL::text AS flag_type,
    CASE WHEN entity_type = 'personality'::text THEN '{"namesake": true}'::jsonb
         ELSE '{}'::jsonb END AS risk_flags
  FROM public.dedup_review_queue q
  WHERE status = 'open'::text
    -- news is fully automatic; it must never surface to a human
    AND entity_type <> 'news'::text;

-- Fail loudly if a later edit undoes either promise.
DO $verify$
DECLARE v_src text;
BEGIN
