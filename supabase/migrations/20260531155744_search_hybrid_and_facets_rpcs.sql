-- Phase 1 RPCs: search_hybrid (FTS+trigram ⊕ pgvector, RRF fused in SQL, trust/liveness/
-- imminence/geo-aware) and search_facets (GROUP BY facet counts). Worker passes an already
-- blended query vector as p_query_vec (bias blend stays Worker-side, matching today).
-- See docs/search-intelligence/meili-to-postgres-migration-plan.md (§3.3).

create or replace function public.search_hybrid(
  p_query          text             default '',
  p_query_vec      extensions.vector(1024) default null,
...
grant execute on function public.search_hybrid(text, extensions.vector, text[], jsonb, double precision, double precision, double precision, timestamptz, int, int) to anon, authenticated, service_role;
grant execute on function public.search_facets(text, text[], jsonb, double precision, double precision, double precision, timestamptz) to anon, authenticated, service_role;
