-- Critical Security Fixes - Phase 1: Database Security (Working Version)
...
-- 2. Add input validation constraints for existing columns
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE table_name = 'profiles' AND constraint_name = 'valid_website_format'
  ) THEN
    ALTER TABLE public.profiles
...
-- 4. Add enhanced security monitoring for sensitive profile operations
