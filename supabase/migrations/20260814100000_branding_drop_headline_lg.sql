-- Drop `text-headline-lg` from the branding override whitelist.
--
-- The token is being removed from the design system: at 2rem it sat 1.14x from
-- `--text-headline` (1.75rem), under the ~1.2x a reader resolves as a separate
-- level, so the two were used interchangeably across its 34 call sites. The
-- heading ranks now step 1.47x / 1.73x / 1.57x / 1.40x.
--
-- This is one of six layers that must move together — src/index.css (both the
-- @theme declaration and the @source safelist), tokenCatalog.ts, branding.ts
-- SIZE_KEYS, src/lib/utils.ts customTextSizes (tailwind-merge), and here.
--
-- SAFE TO DROP: `branding_validate` RAISEs on an unknown key rather than
-- ignoring it, so a persisted doc still carrying `text-headline-lg` would start
-- failing every save AND every edge read. Measured before writing this:
--   site_branding id=1        — published 0 keys, draft 0 keys, no 'headline-lg'
--   site_branding_versions    — 1 row, 0 containing 'headline-lg'
-- The history table matters as much as the live row: branding_revert replays a
-- stored doc back through this function, so a version carrying a dropped key
-- would turn revert into a hard error long after the fact. Neither holds it.
--
-- The body below is 20260809164200's verbatim, minus 'text-headline-lg' from
-- v_size_keys and 'text-headline-lg--line-height' from v_lh_keys. CREATE OR
-- REPLACE needs the whole function, so it is reproduced in full rather than
-- patched.
