-- Kinktionary-corroborated revival, wave 3 of 4 — roles (M–Z).
--
-- WHAT THIS FIXES
--
...
-- Only the 2 row(s) in this wave that carry NO category at all are
...
-- the DO block.
select set_config('app.actor', 'migration:kinktionary-revival-w3', true);
...
begin
  ...
  insert into _rev (slug) values
    ...
    ('mommy'),
    ('monster'),
    ('mummification-bottom'),
    ('mummification-bunny'),
    ('mummification-switch'),
    ('mummification-top'),
    ('mummifier'),
    ('muscle-daddy'),
    ('muscle-mommy'),
  ...
  select count(*) into v_bad
   ...
   where t.id is null;
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % slug(s) absent from unified_tags — disposition file is stale', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % row(s) did not reach the live state', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % indexable row(s) carry no description', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % row(s) carry a retired image_url', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % revived row(s) have no category_id', v_bad;
  end if;
  ...
  select count(*) into v_bad from _rev k
   ...
   where not exists (
     select 1 from public.tag_category_assignments a
      where a.tag_id = t.id and a.category_id = t.category_id and a.is_primary);
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % row(s) have no primary junction row', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w3: % held-back slug(s) are active', v_bad;
  end if;

  select count(*) into v_live from _rev k
    join public.unified_tags t on t.slug = k.slug where t.status = 'active';
  raise notice 'kinktionary revive w3: % of % now active', v_live, (select count(*) from _rev);
end
$mig$;
