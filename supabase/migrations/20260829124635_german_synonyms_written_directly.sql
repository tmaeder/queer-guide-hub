...
--
-- WHAT THAT MIGRATION ASSUMED, AND WHY IT WAS A NO-OP
--
-- It flipped search_synonyms.status from 'approved' to 'active' for the
-- substances vocabulary, on the premise that tag_alias_sync_search_synonym had
-- already created a row per alias. It had not. Measured after that migration
-- applied: the five sample aliases exist, and they have ZERO synonym rows
-- between them; search_synonyms holds 360 rows in total, not thousands.
--
-- The deployed trigger carries a line the migration file that created it does
-- not, added later by 20260910151200:
--
...
-- direct conflict through the alias path: 'approved' buys search and also tags
-- every recipe mentioning Pilze as a psychedelic; 'auto' buys neither.
--
-- AND THE ASSERTION COULD NOT CATCH IT. 20261007160400 asserted that no
-- activate-list row was left 'approved'. With no rows at all that is trivially
-- true, so the migration passed while doing nothing. An assertion phrased as
-- "nothing is left undone" cannot distinguish success from an empty set; this
-- one asserts an absolute expected count instead.
--
-- THE FIX
--
-- Write the synonym rows directly rather than going through the alias gate.
-- This is safe in the one way that matters: run_tag_assignment_reconcile builds
-- its auto-tagging map from unified_tags.name/slug and tag_aliases.alias_name
-- filtered on review_status, and does NOT read search_synonyms at all (verified
...
--
-- Scope is the same unambiguous list 20261007160400 chose, and for the same
-- reason: query expansion has the identical ordinary-word hazard as
-- auto-tagging, aimed at the reader instead of the row. Gras, Schnee, Koks,
-- Pilze, Speed, Ice, Acid and the rest are NOT here and must not be.
...
begin
  ...
  -- passing. This is what 20261007160400 lacked.
