-- Slice 2: add 'friends' and 'travel' modes to people_discovery.
--   friends — discoverable profiles, ranked by compatibility, excluding people
--             the viewer is already friends with (and blocked).
--   travel  — people with active travel intent. With a city/trip context, those
--             heading to that city (travel_mode pin or an overlapping trip);
--             globally, anyone advertising a travel_mode city.
-- 'dating' and 'locals' branches are unchanged from 20260624090000.
-- Operates in auth-uid space (auth.uid() == profiles.user_id).
