-- Curation clears set image_metadata.curated='rejected'; the selector must not
-- re-offer those rows when the enrichment_log cursor is re-opened (first re-run
-- resurrected hand-rejected portraits/dup files). In-place replace, same signature.
