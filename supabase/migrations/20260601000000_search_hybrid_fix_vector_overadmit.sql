-- Fix: search_hybrid admitted ALL documents whenever p_query_vec was non-null
-- (candidate clause `or p_query_vec is not null`). For universal search
-- (p_content_types = null, as the search-proxy issues), this floods the
-- candidate set with the entire corpus (~74k docs), the vector leg ranks news
-- to the top, and a full-table vector scan costs ~3s. Discovered via shadow-mode
-- comparison against Meili (overlap@10 = 0, pg returned all news for venue
-- queries).
--
-- Bound the semantic admission to the top-200 nearest neighbours via the HNSW
-- index (search_documents_embed_hnsw) instead of admitting everything. Keyword
-- matches are still admitted as before; the vector leg now only adds its indexed
-- top-K. Validated: universal venue-vector query 71619 -> 2915 candidates,
-- top-10 all venues (was all news); keyword-only and content-type-filtered paths
-- byte-identical to prior behaviour; latency 3004ms -> 730ms.
...
drop function if exists public.search_hybrid_v2(text, vector, text[], jsonb, double precision, double precision, double precision, timestamptz, integer, integer);
