-- Applied live via MCP apply_migration (version stamped by the call, 20260819181127);
-- this file mirrors that exact version so CI `db push` matches it as already-applied
-- and skips it, per the established repair-shim convention.
--
-- Adds a 'film' subcategory group (Film/DVD/Blu-ray/Cinema/Documentary) and a
-- 'calendars' group, both mapped to the existing 'books_art' department, so the
-- salzgeber.shop + pro-fun.de marketplace imports (predominantly film media) don't
-- fall through to 'other'. marketplace_subcategory_group() previously had no film
-- keyword at all — see CLAUDE.md marketplace taxonomy notes.
...
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  ...
  SELECT CASE
    ...
    WHEN n ~ '\y(films?|movies?|dvds?|blu ?rays?|cinema|documentaries?)\y'          THEN 'film'
    WHEN n ~ '\y(books?|magazines?|zines?|comics?|novels?|ebooks?)\y'               THEN 'books'
    WHEN n ~ '\y(calendars?)\y'                                                     THEN 'calendars'
