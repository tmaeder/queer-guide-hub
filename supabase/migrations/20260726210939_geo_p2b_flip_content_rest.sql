-- P2 redo B3: remaining content entities. personalities keeps ONLY the
-- birth_city computed rel (already created) — no bare cities()/countries()
-- rels there because the table has two city and two country FKs (ambiguous
-- semantics); nothing embeds them bare today.
