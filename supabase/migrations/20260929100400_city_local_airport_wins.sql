-- The airport inside the city becomes the primary one.
--
-- 20260929100300 partitioned the codes into local and nearby but deliberately
-- left the ranking alone, so Cologne kept Düsseldorf as its primary while CGN
-- sat in `local_airport_codes` one row below. That was the conservative call and
-- the reasoning is still on the record; the product decision is the other way,
-- and this migration makes `is_local` the first ranking key.
--
-- What it buys, measured on prod: Cologne DUS -> CGN, Liverpool MAN -> LPL,
-- Dortmund DUS -> DTM, San Jose SFO -> SJC, Antwerp BRU -> ANR, Rotterdam
-- AMS -> RTM, Hamilton YYZ -> YHM. Roughly 97 cities carried a primary that was
-- not their own airport while their own airport was already in the list.
--
-- What it costs, and this is not hidden: a city whose own airport is the SMALL
-- one now leads with it. Dallas gets Love Field instead of DFW (whose
-- municipality reads "Dallas-Fort Worth", so DFW does not count as local),
-- Taipei gets Songshan instead of Taoyuan, Nagoya Komaki instead of Centrair,
-- Bucharest Băneasa instead of Otopeni, Kobe UKB instead of Kansai. No signal
-- available here separates "the city's own small field" from "the metro
-- gateway" -- the choice is which way to be wrong, and being wrong towards the
-- airport that is actually in the city is the decision taken.
--
-- `major_airport_code` feeds the Aviasales flight search
-- (src/hooks/useTripBookingContext.ts), so this changes which airport a reader
-- is offered flights to. `airport_codes` still carries every candidate in rank
-- order, and `nearest_airport_codes` still names the hub, so nothing is lost
-- from the page -- only the headline changes.
--
-- Cities with no local airport are untouched: Essen has none, so DUS stays its
-- primary and the page keeps saying "nearest airport DUS, 25 km".
    ...
    IF r.latitude IS NOT NULL AND r.longitude IS NOT NULL AND r.country_code IS NOT NULL THEN
      ...
      FROM (
        SELECT array_agg(b.iata_code ORDER BY b.rk) FILTER (WHERE b.rk <= 4)        AS codes,
               ...
               -- Tied to nearest_codes[1], not to "any non-local candidate":
               -- London's four airports are all municipality "London", so LTN
               -- falls outside the top 4 and the column would otherwise report
               -- 44 km with no code to attach it to.
               round(min(b.dist_km) FILTER (WHERE NOT b.is_local
                                              AND b.rk <= 4)::numeric, 1)          AS nearest_km,
        ...
        FROM (
          SELECT k.iata_code, k.name, k.dist_km, k.is_local,
                 ...
                 (k.is_local = first_value(k.is_local) OVER w
                  AND k.sitelinks IS NOT DISTINCT FROM first_value(k.sitelinks) OVER w
          ...
          -- is_local FIRST. See the header: an airport inside the city wins,
          -- and prominence only orders what is left.
          WINDOW w AS (ORDER BY k.is_local DESC,
...
-- Re-queue the corpus: the ranking changed, so every stamped row needs a second
-- look. The nightly cron drains it at 300 a night; the operator can drain it in
-- one go with repeated `SELECT public.run_city_airport_link(600);`.
