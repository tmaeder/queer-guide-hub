-- commit_venue_staging_item wrote two values its own CHECK constraints reject
--
-- Both faults are invisible until commit: pipeline-validate approves the row,
-- then the INSERT trips a constraint and the row lands disposition='rejected'
-- carrying a raw Postgres message. Measured 2026-08-22 across the venue
-- sources: refuge-restrooms had lost 907 of 1,851 rows (49%), osm 203 of 381
-- (53%), community-manual 13 of 13.
--
-- 1. country. venues_country_iso2_check is
--    `country IS NULL OR country ~ '^[A-Z]{2}$'` — NULL is allowed, the EMPTY
--    STRING is not. The function wrote `coalesce(nullif(...,''), '')`, turning
--    an absent country into exactly the one value the constraint forbids, so a
--    source that supplies no country could never commit a row. Live evidence
--    that NULL is the intended representation: 7,103 venues have country IS
--    NULL and ZERO have ''.
--
-- 2. category. The default was 'unknown', which is not a member of
--    venues_category_check at all — a row with no category was guaranteed to
--    be rejected rather than merely uncategorised. Worse, a present-but-INVALID
--    value failed identically, and that is the bigger half: osm shipped
--    'community-center' (hyphen, the member is 'community_center') and
--    community-manual shipped 'nightclub' (the member is 'club'). Now the value
--    is separator-normalised and anything still unrecognised degrades to
--    'other'.
--
-- Source-side fixes ship alongside this (_shared/venue-category.ts, wired into
-- source-osm-venue and source-refuge-restrooms) and are where semantic mapping
-- belongs, because it is testable there. This migration is the backstop that
-- makes the class non-fatal: a wrong category becomes 'other' and a missing
-- country becomes NULL, instead of silently dropping the row.
--
-- Verified by replaying 8 real rejected rows against the patched function in a
-- rolled-back transaction: 8/8 commit.
