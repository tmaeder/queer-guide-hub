-- Add the derived LGBTQ-relevance boost to the served search_hybrid. Venues use
-- their graded lgbtq_score; non-venue entities (events/news/villages/tags/cities)
-- are curated-queer by construction so are treated as fully relevant. Only change
-- vs the prior definition: + sd.lgbtq_score into cand, + one scoring term. Output
-- shape unchanged (search_hybrid contract preserved; see assert_search_hybrid_contract).
...
-- remove the shadow-era scratch candidates left in the DB during evaluation
...
drop function if exists public.search_hybrid_lgbtq(text, vector, text[], jsonb, double precision, double precision, double precision, timestamptz, integer, integer);
