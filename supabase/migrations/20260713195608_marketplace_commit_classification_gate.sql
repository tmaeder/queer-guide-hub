-- ============================================================================
-- Marketplace commit: require relevance classification before commit
-- ----------------------------------------------------------------------------
-- commit_marketplace_staging_batch selected pending+validated rows WITHOUT
-- requiring classification_result, so staging rows that had not yet passed the
-- marketplace-relevance LLM gate could be committed to live marketplace_listings
-- (observed 2026-07-13: ~1,750 unclassified rows committed in one drain burst,
-- bypassing the LGBTQ+ relevance + sensitivity gate entirely).
--
-- The marketplace DAG runs relevance BEFORE commit, and refresh-mode re-opened
-- rows preserve classification_result — so requiring it here breaks no legit
-- path; it only stops the gate bypass. Unclassified rows simply wait for the
-- relevance drain to classify (or reject) them.
--
-- Applied live 2026-07-13 via MCP apply_migration (version 20260713195608);
