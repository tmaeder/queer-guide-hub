...
--
-- CORRECTION, re-measured 2026-09-14. The "never applied" line above was true
-- when it was written and is NOT true now: BOTH versions are in
-- schema_migrations today, each recorded with the full five-statement body
-- (20261027120000 8,226 chars; 20261028120000 8,685). So this version did
-- reach prod with its original body at some point after the emptying was
-- written. The conclusion is unchanged — the work is done and re-running it
-- is pointless — but do not cite the zero above as current.
--
-- The emptying was also INCOMPLETE, which is why this file has been
-- unparseable ever since: the no-op block below replaced the opener
-- (`do $verify$ declare ... begin`) while ~70 lines of its plpgsql body were
-- left behind as bare text after `end $$;`, ending in an `end $verify$;` whose
-- opener no longer existed. `db push` stops at the first file it cannot parse
-- and strands every migration queued behind it, so that residue was a live
-- hazard rather than clutter. Removing it finishes the emptying as intended.
