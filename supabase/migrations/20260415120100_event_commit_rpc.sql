-- Event commit RPC
-- Atomic per-item commit: upsert event + event_sources + audit + advisory lock.
-- Paired with 20260415120000_event_data_ops_foundation.sql.
...
CREATE OR REPLACE FUNCTION public.commit_event_staging_batch(p_limit INT DEFAULT 50)
RETURNS TABLE(staging_id UUID, event_id UUID, action TEXT)
LANGUAGE plpgsql AS $$
DECLARE
  r RECORD;
  res RECORD;
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      VALUES (r.id, 'commit', 'rejected', 'batch',
              jsonb_build_object('error', SQLERRM, 'target_table', 'events'));
    END;
  END LOOP;
END;
$$;

GRANT EXECUTE ON FUNCTION public.commit_event_staging_batch(INT) TO authenticated, service_role;
