-- Extend search_hybrid with real date-range + price-range filters and a server-
-- side sort (p_sort). Previously date/price filters were dropped on the floor and
-- sorting was client-side (only ordered the already-fetched page — wrong across
-- pagination). New optional params are appended with DEFAULT NULL so existing
-- callers (worker pgSearch, trip-concierge / ai-plan-trip edge fns) keep working.
-- Everything else (RRF fusion, boosts, target_groups/is_free filters, output
-- shape) is preserved from 20260602170100.
drop function if exists public.search_hybrid(text, vector, text[], jsonb, double precision, double precision, double precision, timestamptz, integer, integer);
...
cand as (
  ...
  where (p_content_types is null or sd.entity_type=any(p_content_types))
    ...
    -- date window (only narrows dated docs; undated excluded when a bound is set)
    ...
    -- price overlap (priced docs only; pricless excluded when a bound is set)
...
-- The contract guard pins the exact signature via ::regprocedure — repoint it at
-- the extended arg list so assert_search_hybrid_contract() keeps passing.
