...
COMMENT ON FUNCTION public.staging_unreachable_stats() IS
  'news_articles staging rows no live consumer can select: disposition=pending, enrichment_status=completed, scored by quality-score but never seen by quality-enhance. Deliberately news-only — enrichment_status gates commit for news alone. recent_24h is a zero-invariant for NEW stranding — read by scripts/check-pipeline-health.mjs. See 20261206100100.';
