-- Wave A.1: visibility queries axis incorporates cluster membership
--
-- Updates compute_visibility_score so the queries axis (placeholder 0.5
-- since #148) reflects cluster membership — entities that belong to one
-- or more active topic_clusters get a score bump. Same return shape;
-- same axis weights. Search-log integration remains future work.
--
-- New queries-axis logic:
--   no clusters      -> 0.5  (unchanged baseline)
--   1 cluster        -> 0.6
--   2 clusters       -> 0.7
--   3 clusters       -> 0.8
--   4 clusters       -> 0.9
--   5+ clusters      -> 1.0
-- Plus a "matching active synonyms whose terms appear in the title"
-- contribution remains in the synonyms axis (unchanged).
--
-- Inline cluster-count query (does not depend on entity_cluster_ids
-- helper from #171) so this migration can land independently.
...
begin
  ...
  -- ── Queries axis (cluster membership) ────────────────────────────────────
  -- Replaces the static 0.5 placeholder. Inline cluster-count via tags so
  -- this migration is independent of the entity_cluster_ids helper.
  if v_assign_type is null then
    v_query := 0.5;
    n_query := array_append(n_query, 'cluster membership not applicable for this entity type');
  else
    select count(distinct tc.id)::int into v_cluster_count
      from public.unified_tag_assignments uta
    ...
    if v_cluster_count = 0 then
      v_query := 0.5;
      n_query := array_append(n_query, 'not in any active topic cluster');
      v_suggestions := array_append(v_suggestions, 'Add this entity''s tags to one or more topic_clusters for facet visibility');
    else
      v_query := least(0.5 + (v_cluster_count::numeric * 0.1), 1.0);
      n_query := array_append(n_query, format('member of %s topic cluster(s)', v_cluster_count));
    end if;
  end if;
...
comment on function public.compute_visibility_score(text, uuid) is
  'Per-axis Search Visibility Score. Queries axis incorporates topic_cluster membership (#154); search-log popularity remains future work.';
