...
-- active tags -- ~13.8M plpgsql-ish calls -- and hit the statement timeout.
-- On the nightly cron a timeout is a full rollback, so the job would have
-- appeared "registered and never once succeeded".
--
...
-- set, ~40ms. tag_plural_of() is kept as the documented predicate for
-- tag_slugs_are_variants() and for one-off checks; it is just no longer in a
-- join.
