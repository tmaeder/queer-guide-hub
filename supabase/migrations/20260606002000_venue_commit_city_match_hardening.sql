-- Prevention: harden the venue-commit city match.
-- Previously: `SELECT id FROM cities WHERE lower(name)=lower(city) LIMIT 1` —
-- name-only, no country scope, and it could bind a venue to a placeholder
-- ("tmp-") bucket city. Once a venue had a (wrong) city_id, geo-link-content
-- (which only reprocesses NULL city_id) would never re-fix it.
-- Now: country-scoped, placeholder-excluded, population-ordered, with a
-- name-only fallback that still excludes tmp- stubs. When only a placeholder
-- would match, city_id is left NULL so the hourly geo-link pass resolves it.
-- Only the city-match block (was lines 91-94) changed vs 20260414260000.
...
BEGIN
  ...
  IF v_loc->>'city' IS NOT NULL AND length(btrim(v_loc->>'city')) > 0 THEN
    ...
    -- Fallback: name-only, still excluding placeholder stubs.
