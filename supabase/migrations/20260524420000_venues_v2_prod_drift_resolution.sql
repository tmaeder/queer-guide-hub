-- Capture the prod state of /venues v2 that was applied via MCP after the
-- earlier migrations (20260524160000, 20260524180000) landed. Two things
-- drifted:
--   1. `rpc_venues_ranked` was rewritten for perf — the LEFT JOIN to
--      `venue_checkin_stats` was dropped (it forced full-table aggregation
--      before LIMIT and tripped the 8 s anon statement_timeout). The function
--      also became SECURITY DEFINER so it can read venues + checkin tables
--      without depending on the caller's role grants.
--   2. `venue_leaderboard_city` / `_global` MV SELECT grants for anon /
--      authenticated were re-applied; the original grants in 20260524160000
--      were not preserved across some intervening MV refresh.
--
-- This migration is idempotent — running it against a fresh DB produces the
-- same end-state as prod, and against prod it's a no-op (REPLACE + GRANT).
...
DECLARE
  ...
  v_total             bigint;
BEGIN
  -- Cheap filtered count first so we can return it on every row without
  -- forcing a COUNT(*) OVER () over the full scored CTE.
