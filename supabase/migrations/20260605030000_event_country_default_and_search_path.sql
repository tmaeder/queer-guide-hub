-- 1. Stop defaulting an event's country to 'US' when the source omits it. The
--    default propagated into geo-link-content (which resolves country from the
--    text field) and mislinked non-US events with a missing country to US cities.
--    Use '' instead so geo-linking resolves only from the city.
-- 2. Restore search_path hardening on commit_venue_staging_item (the
--    20260605000000 redefinition dropped the pin added by the security-lint
--    migration) and pin the new coerce_null_island_coords trigger fn.
...
-- Restore / add search_path pins (lint: function_search_path_mutable)
ALTER FUNCTION public.commit_venue_staging_item(uuid, text) SET search_path = public, pg_temp;
ALTER FUNCTION public.coerce_null_island_coords() SET search_path = public, pg_temp;
