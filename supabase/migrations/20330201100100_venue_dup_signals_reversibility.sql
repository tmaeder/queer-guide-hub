...
-- 20330201100000 wrote, and that migration asserts it at deploy time. Neither
...
COMMENT ON FUNCTION public.venue_dup_signals() IS
  ...
  'undone — the live _venue_merge_core has drifted from 20330201100000). Warns on a '
...
BEGIN
  ...
  IF (v->>'merges_pre_schema_total')::int = 0 THEN
    RAISE EXCEPTION 'merges_pre_schema_total is 0 — expected the ~1,544 rows predating 20330201100000; the key is not reading the audit table';
  END IF;
