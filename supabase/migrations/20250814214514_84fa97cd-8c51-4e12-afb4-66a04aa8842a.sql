...
-- Remove SECURITY DEFINER from get_entity_attributes and get_entity_tags functions
...
-- 1. Removed SECURITY DEFINER from table-returning functions that don't need elevated privileges
-- 2. These functions now rely on RLS policies for access control
-- 3. Access to underlying data is controlled by the existing RLS policies on attributes, tags, and assignment tables
-- 4. This eliminates the "Security Definer View" linter warnings while maintaining security