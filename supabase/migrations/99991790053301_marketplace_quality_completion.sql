...
UPDATE public.admin_automations SET
  enabled=true,consecutive_failures=0,last_run_status=NULL,schedule='* * * * *',
...
UPDATE public.admin_automations SET
  ...
  action=jsonb_build_object('type','cron','jobname','marketplace_description_enhance','command',$cmd$
    SELECT net.http_post(
      ...
      headers:=jsonb_build_object('Content-Type','application/json','X-Internal-Secret',
        (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name='internal_invoke_secret')),
      body:='{"batch_size":50}'::jsonb,timeout_milliseconds:=120000);
  $cmd$),updated_at=now()
...
UPDATE public.admin_automations SET schedule='* * * * *',
  action=jsonb_set(action,'{command}',to_jsonb(replace(action->>'command',
    '"batch_size":15','"batch_size":30'))),updated_at=now()
...
UPDATE public.marketplace_listings SET taxonomy_model_status='pending',taxonomy_model_attempts=0
WHERE status='active' AND subcategory_group='other' AND taxonomy_model_status='failed';
...
DECLARE v_active integer; v_attr integer; v_stale integer; v_healthy integer;
  v_tax_pending integer; v_other integer; v_desc_recovery integer; v_changed boolean:=false;
BEGIN
