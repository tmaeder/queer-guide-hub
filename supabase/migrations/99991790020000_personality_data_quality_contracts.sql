...
-- Preserve every workflow sentinel or malformed identifier before clearing it
...
select 'personality-qid-sentinel-v1', id, 'wikidata_qid', to_jsonb(wikidata_qid), null,
       'workflow state or malformed value is not a Wikidata identifier'
from public.personalities
where wikidata_qid is not null and wikidata_qid !~ '^Q[0-9]+$'
on conflict do nothing;

update public.personalities
set wikidata_status = case
      when wikidata_qid ~ '^Q[0-9]+$' then 'resolved'
      when wikidata_qid like 'SKIP_%' or upper(wikidata_qid)='NOT_FOUND' then 'not_found'
      else 'needs_review'
    end,
    wikidata_qid = case when wikidata_qid ~ '^Q[0-9]+$' then wikidata_qid else null end
where wikidata_status is null
   or (wikidata_qid is not null and wikidata_qid !~ '^Q[0-9]+$');
...
from public.personalities p
cross join lateral jsonb_array_elements_text(
  case when jsonb_typeof(p.fields)='object'
             and jsonb_typeof(p.fields->'parties')='array'
       then p.fields->'parties' else '[]'::jsonb end
) party(value)
where btrim(party.value) <> ''
on conflict do nothing;
...
insert into private.personality_remediation_audit
  (batch_key, personality_id, field_name, old_value, new_value, reason)
select 'personality-fields-profile-v1', p.id, 'profile_url', to_jsonb(p.profile_url),
       to_jsonb(btrim(p.fields->>'pornhub_profile')),
       'adult platform profile migrated from legacy fields object'
from public.personalities p
where jsonb_typeof(p.fields)='object'
  and nullif(btrim(p.fields->>'pornhub_profile'),'') is not null
  and p.profile_url is null
on conflict do nothing;

update public.personalities p
set profile_url=btrim(p.fields->>'pornhub_profile'), updated_at=now()
where jsonb_typeof(p.fields)='object'
  and nullif(btrim(p.fields->>'pornhub_profile'),'') is not null
  and p.profile_url is null;
...
insert into private.personality_remediation_audit
  (batch_key, personality_id, field_name, old_value, new_value, reason)
select 'personality-fields-objects-v1', p.id, 'fields', p.fields, '[]'::jsonb,
       'legacy fields object migrated to normalized affiliation/profile storage'
from public.personalities p
where jsonb_typeof(p.fields)='object'
on conflict do nothing;
