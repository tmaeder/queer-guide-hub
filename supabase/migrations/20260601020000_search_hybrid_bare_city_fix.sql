-- Fix bare-city full-scan introduced by the geo soft-boost migration.
-- A bare city query ("sitges", "berlin") detects the city and strips it from
-- the keyword query, leaving eff_q NULL. The previous admission clause treated
-- NULL eff_q as "match everything", so the candidate set became the entire
-- corpus (sitges -> 23188 venues) ranked only by the city boost — slow + noisy.
--
-- Fix: when eff_q is NULL but a city was detected, restrict candidates to that
-- city (bare-city browse). Empty query with no city still browses all. Queries
-- with a non-NULL eff_q are byte-identical to before (no broad-query change).
-- Validated: sitges 23188 -> 53 (all Sitges); broad/geo/typo queries unchanged.
--
-- NOTE (not fixed here): broad common-token queries ("gay bars in berlin") cost
-- ~1.7s in PG — ranking large candidate sets + the similarity()>0.2 trigram
-- branch (seq scan; the % operator needs pg_trgm.similarity_threshold which this
-- role can't SET, and at the default 0.3 it's slower for common phrases). This
-- is a fundamental PG-hybrid-search cost, the remaining cutover blocker, and
-- needs a strategy decision (candidate cap / popular-query cache / Meili for
-- broad + PG for specific), not an index tweak.
...
scored as (select f.*, f.rrf + case when f.boost_city is not null and lower(f.city)=f.boost_city then 0.05 else 0 end + case when f.is_featured then 0.02 else 0 end + case when f.liveness_status in ('dead','cancelled','dead_link','sold_out') then -0.5 when f.liveness_status='live' then 0.01 else 0 end + case when f.closed_at is not null then -0.5 else 0 end + case when f.entity_type='event' and f.start_date is not null and f.start_date>=p_now then 0.03*exp(-extract(epoch from (f.start_date-p_now))/(60*60*24*30)) else 0 end - case when f.dist_m is not null then least(f.dist_m/50000.0,1)*0.02 else 0 end score from fused f)
...
drop function if exists public.search_hybrid_v5(text, vector, text[], jsonb, double precision, double precision, double precision, timestamptz, integer, integer);
