-- RENUMBERED from 20261007130000. Merged in #3108 and never applied: six other
-- migrations sorted below prod's head and `supabase db push` aborts on the first
-- of those, so from 2026-08-29 10:24Z nothing reached prod at all. By the time
-- that jam cleared (#3118 and siblings) the repo head had passed this file's
-- version, stranding it in turn. Content revised below where the world moved,
-- not merely re-dated.
--
...
--
-- Three sessions reached this same diagnosis on 2026-08-29 within a few hours,
-- and `tag_category_text_mirror_reconciler` got there first: it is APPLIED on
-- prod, and it registered `tag_category_text_resync` in `admin_automations` and
-- scheduled it at `55 3 * * *` calling `run_tag_category_resync(500)`. Verified
-- live, not assumed — `select jobname, schedule, command from cron.job where
-- jobname ilike '%tag_category%'` returns exactly that one row.
--
...
-- that migration did NOT do is touch `run_tag_category_resync` itself — it calls
-- the existing body in a loop — so both defects in that body are still live and
-- are now scheduled to run nightly. Correcting the function the new cron calls
-- is what remains, and it is all this migration does.
--
-- The fill-only guard is therefore FORWARD-LOOKING only. The rows it would have
-- protected are already gone: that migration's loop ran the unguarded predicate
-- and erased them, and the cohort measured at 11 in the morning read 0 by
-- midday. Its own assertions could not see that — they check only rows that HAVE
-- a primary junction — which is exactly why the guard belongs in the function
-- rather than in any one migration's post-conditions.
--
...
-- `20261007160000_tag_category_id_backfill_from_junction`, and it adds
...
-- its pair — the panel drift test reads the LAST migration defining the function
-- — so this migration does not touch it.
...
-- the same function is duplication, not redundancy-as-safety. The assertion
-- below checks that schedule EXISTS rather than creating one, so a revert of
-- that migration surfaces as a failure instead of leaving the function corrected
...
begin
  ...
  -- post-conditions. A migration assertion that depends on state another
  -- migration may legitimately have cleared is a landmine in `db push`, and
  -- probing it by inserting a throwaway tag would fire the slug, search and
  -- adult-recompute triggers to prove a one-line predicate. The guard is a
  -- property of the FUNCTION TEXT, so it is gated where that can be read
  -- directly and for free: `src/lib/__tests__/tagCategoryMirror.test.ts` fails
  -- if `want is not null` leaves the definition.
