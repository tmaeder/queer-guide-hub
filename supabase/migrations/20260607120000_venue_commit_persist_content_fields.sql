-- Venue commit was silently dropping enrichment fields.
--
-- commit_venue_staging_item folded consensus winners into normalized_data, but its UPDATE
-- branch only wrote description/address/phone/email/website/coords/city_id. hours, tags,
-- images, category, and lgbti_relevance_score never landed on the venue — so consensus on
-- those was recorded as provenance and then thrown away. This blocked the entire venue
-- enrichment loop (the refresh/re-fetch path stages re-fetched content as a merge_candidate
-- and relies on commit to persist it).
--
-- Fix: persist these fields on BOTH branches. Scalars (hours/category/relevance) fill-empty
-- via coalesce — never overwrite a curated value. Arrays (tags/images) union losslessly
-- (consensus already unions across sources upstream, so a fill-empty would drop merges).
-- Everything else is byte-identical to 20260606002000 (the city-match hardening).
...
BEGIN
  ...
  -- Content fields newly persisted by this migration.
  ...
  -- City resolution: country-scoped, placeholder-excluded, population-ordered.
  ...
  ELSE
    UPDATE public.venues SET
      ...
      -- lossless array union (consensus already unions across sources upstream)
