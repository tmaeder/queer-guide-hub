...
-- directions, so the pass in 20270301101000 does not have to be run a third time.
--
...
-- merge irreversible — including all nine merged by 20270301101000, whose
...
begin
  ...
  -- since 20270301101100 that state is refused outright, so the narrow form
...
begin
  ...
  if v_owner is not null then
    raise exception
      ...
      using hint = 'Delete the tag_aliases row (and its search_synonyms row FIRST -- that FK is ON DELETE SET NULL) in the same transaction, or merge the two tags instead. See 20270301101000.';
  end if;
  return NEW;
end;
$function$;
...
begin
  ...
  -- The seal cannot be added while the corpus violates it. 20270301101000 is
  ...
  if v_bad > 0 then
    raise exception 'tag shadow seal: % shadowing alias(es) still exist — 20270301101000 must apply first', v_bad;
  end if;
  ...
  -- 20270301101000 just made while the seal is live. Without the widening this
  ...
  if v_audit is null then
    raise exception 'tag shadow seal: no reversible pass-2 merge to probe with — 20270301101000 must apply first';
  end if;
