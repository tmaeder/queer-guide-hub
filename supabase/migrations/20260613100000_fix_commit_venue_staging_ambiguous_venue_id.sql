-- Fix: commit_venue_staging_item raised "column reference \"venue_id\" is ambiguous"
-- whenever a staging row carried a source_entity_id. The function RETURNS TABLE(venue_id uuid, ...),
-- so the OUT column `venue_id` collided with venue_sources.venue_id in the existing-venue lookup.
-- This blocked every commit for sources that set source_entity_id (e.g. the nude-places import).
-- Minimal fix: alias the table and qualify the column. Body otherwise unchanged from the deployed def.
