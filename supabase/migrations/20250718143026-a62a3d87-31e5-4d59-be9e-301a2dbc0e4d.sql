-- Clean up tag system redundancies
...
-- Insert unique tags from content_tags
...
-- Insert unique tags from existing tags table if it exists
INSERT INTO public.unified_tags (name, slug, description, color, image_url, usage_count, category)
SELECT DISTINCT name, slug, description, color, image_url, usage_count, 'general' as category
FROM public.tags
ON CONFLICT (slug) DO NOTHING;
...
JOIN public.unified_tags ut ON ut.slug = t.slug
ON CONFLICT (tag_id, entity_id, entity_type) DO NOTHING;
