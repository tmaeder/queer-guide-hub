-- ============================================================================
-- run_city_safety_backfill wrote to a VIEW with ON CONFLICT — impossible.
-- ----------------------------------------------------------------------------
-- The job failed six consecutive nights (2026-08-11 .. 2026-08-16) with
-- "there is no unique or exclusion constraint matching the ON CONFLICT
-- specification" and nothing said so until run tracking landed (#2817). It was
-- then auto-paused, which is the safety net working.
--
-- CAUSE. 20260801130000 (B1, one review queue instead of five) renamed
-- city_review_queue to city_review_queue_legacy, created the unified
-- public.entity_review_queue, and put an auto-updatable compat VIEW back under
-- the original name. The view carries `ALTER VIEW ... ALTER COLUMN entity_type
-- SET DEFAULT 'city'` precisely so the enrichers could keep INSERTing without
-- naming the new column — and for a plain INSERT that works.
--
-- ON CONFLICT is the one thing it cannot carry. A view has no indexes, so
-- there is no arbiter to infer, and PostgreSQL rejects the statement outright.
-- run_city_safety_backfill is the ONLY function in the schema that inserts
-- into any of the five compat views (verified against the live catalog:
-- batch_approve_cited_city_ratings, batch_approve_safe_city_reviews and
-- triage_stuck_city_safety_reviews only UPDATE, which auto-updatable views do
-- support). So the blast radius was exactly one job, and it is this one.
--
-- FIX. Write to public.entity_review_queue directly, naming entity_type, with
-- the arbiter that actually exists:
--   uq_erq_open ON (entity_type, entity_id, field) WHERE status = 'open'
-- The predicate has to be repeated in the ON CONFLICT clause — the index is
-- partial and cannot be inferred from the column list alone.
--
-- The NOT EXISTS in the selection loop moves to the base table too. It worked
-- through the view, but leaving one read on the view and one write on the
-- table is how the next reader concludes the view is still the write path.
--
-- Nothing else changes: same composer, same tiering, same outing-safety
-- invariant. compose_safety_note still forces auto_publishable = false for any
-- criminalizing or death-penalty destination, so those still land in the queue
-- for a human, and review_field_registry independently carries
-- risk_gate = 'criminalizing_destination' for ('city','safety_notes').
-- ============================================================================
...
COMMENT ON FUNCTION public.run_city_safety_backfill(integer, boolean) IS
  'Composes deterministic safety notes for cities. Auto-publishes only low-risk, high-equality destinations; everything else queues in entity_review_queue for a human. Writes the BASE table, not the city_review_queue compat view — a view cannot carry ON CONFLICT.';
