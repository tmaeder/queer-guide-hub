-- Kinktionary-corroborated revival, wave 1 of 4 — kink activities, sexual activities, philia/fetish, toys, porn, play spaces.
--
-- WHAT THIS FIXES
--
...
-- Only the 0 row(s) in this wave that carry NO category at all are
...
-- the DO block.
select set_config('app.actor', 'migration:kinktionary-revival-w1', true);
...
begin
  ...
  select count(*) into v_bad
   ...
   where t.id is null;
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % slug(s) absent from unified_tags — disposition file is stale', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % row(s) did not reach the live state', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % indexable row(s) carry no description', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % row(s) carry a retired image_url', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % revived row(s) have no category_id', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % row(s) have no primary junction row', v_bad;
  end if;
  ...
  if v_bad > 0 then
    raise exception 'kinktionary revive w1: % held-back slug(s) are active', v_bad;
  end if;

  select count(*) into v_live from _rev k
    join public.unified_tags t on t.slug = k.slug where t.status = 'active';
  raise notice 'kinktionary revive w1: % of % now active', v_live, (select count(*) from _rev);
end
$mig$;
