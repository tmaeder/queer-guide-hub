...
WITH CHECK (
  ...
  session_token IS NOT DISTINCT FROM OLD.session_token AND
  session_token_encrypted IS NOT DISTINCT FROM OLD.session_token_encrypted AND
  ip_address IS NOT DISTINCT FROM OLD.ip_address AND
  ip_address_encrypted IS NOT DISTINCT FROM OLD.ip_address_encrypted AND
  user_agent IS NOT DISTINCT FROM OLD.user_agent AND
  user_agent_encrypted IS NOT DISTINCT FROM OLD.user_agent_encrypted AND
  encryption_key_id IS NOT DISTINCT FROM OLD.encryption_key_id
);
...
WITH CHECK (
  ...
  is_active = false AND
  -- Prevent modification of other fields during deactivation
  session_token IS NOT DISTINCT FROM OLD.session_token AND
  session_token_encrypted IS NOT DISTINCT FROM OLD.session_token_encrypted AND
  ip_address IS NOT DISTINCT FROM OLD.ip_address AND
  ip_address_encrypted IS NOT DISTINCT FROM OLD.ip_address_encrypted AND
  user_agent IS NOT DISTINCT FROM OLD.user_agent AND
  user_agent_encrypted IS NOT DISTINCT FROM OLD.user_agent_encrypted
);
...
PERFORM public.log_enhanced_security_event(
  'SESSION_RLS_SECURITY_FIX',
  NULL,
  jsonb_build_object(
    'description', 'Fixed user_sessions RLS policies to prevent session hijacking',
    'changes', jsonb_build_array(
      'Replaced overly restrictive policy',
      'Added proper access controls for users own sessions',
      'Protected sensitive encrypted data from exposure',
      'Added admin monitoring capabilities',
      'Implemented secure session validation function'
    ),
    'timestamp', now()
  ),
  'high'
