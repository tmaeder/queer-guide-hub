...
-- 2. Users can view their own photos OR public photos based on profile privacy
...
USING (
    ...
    (is_public = true AND EXISTS(  -- Public photos only if profile allows it
    ...
    (is_public = true AND EXISTS(  -- Friends-only photos if users are connected
        ...
        AND EXISTS(
            ...
            WHERE ((ur.user_id = user_photos.user_id AND ur.related_user_id = auth.uid()) OR
                   (ur.user_id = auth.uid() AND ur.related_user_id = user_photos.user_id))
