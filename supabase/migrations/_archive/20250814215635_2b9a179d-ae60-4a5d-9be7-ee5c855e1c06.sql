...
-- Focus only on existing functions that need search path fixes
...
-- Fix critical functions that don't have SET search_path TO ''
-- These are the ones causing the "Function Search Path Mutable" warnings
...
CREATE OR REPLACE FUNCTION public.is_group_member_or_admin(group_id_param uuid, require_admin boolean DEFAULT false)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
-- User role checking (this is critical for security)
...
-- Update other critical functions to have proper search path
