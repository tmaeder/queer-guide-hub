-- PHASE 1: CRITICAL DATA PROTECTION - RLS POLICY HARDENING (Final)
...
CREATE POLICY "profiles_admin_access_with_logging" ON public.profiles
  FOR SELECT 
  TO authenticated
  USING (
    EXISTS (
...
-- Harden user_photos policies with proper friend access
...
CREATE POLICY "user_photos_friends_access" ON public.user_photos
  ...
  USING (
    ...
    EXISTS (
      ...
      WHERE ((ur.user_id = auth.uid() AND ur.target_user_id = user_photos.user_id) 
          OR (ur.target_user_id = auth.uid() AND ur.user_id = user_photos.user_id))
...
-- Set default privacy for user photos to private
