...
-- 2. Users can view their own photos OR public photos with proper permission
...
USING (
    ...
    (is_public = true AND EXISTS(  -- Public photos with profile privacy check
        SELECT 1 FROM public.profiles p 
        WHERE p.user_id = user_photos.user_id 
        AND p.photos_visibility = 'public'
    )) OR
    (is_public = true AND EXISTS(  -- Friends-only photos if users are friends
        SELECT 1 FROM public.profiles p 
        WHERE p.user_id = user_photos.user_id 
        AND p.photos_visibility = 'friends'
        AND EXISTS(
            SELECT 1 FROM public.user_relationships ur
            WHERE ((ur.user_id = user_photos.user_id AND ur.friend_id = auth.uid()) OR
                   (ur.user_id = auth.uid() AND ur.friend_id = user_photos.user_id))
