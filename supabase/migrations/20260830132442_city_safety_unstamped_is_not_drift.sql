-- Phase 2 part D, correction: AN UNSTAMPED NOTE IS NOT A DRIFTED NOTE.
--
-- 20260830132243 (applied minutes earlier) collapsed two different states into one
-- boolean. `field_provenance.safety_notes.facts IS DISTINCT FROM <current>` is true both
-- when the stamped facts genuinely CHANGED and when there are no stamped facts at all.
-- The first is evidence the note is wrong. The second is absence of evidence — and the
-- retraction branch treated them identically, so it would blank a perfectly correct note
-- purely because it predated the stamp.
--
-- Measured before this correction: 4,699 derived notes, ALL unstamped, and ZERO of them on
-- a country scoring < 75 — so the auto-publish branch would have caught every one and
-- nothing would actually have been retracted today. The defect is LATENT, not active. It
-- becomes active the moment any country with published derived notes drops below the
-- auto-publish bar or turns criminalising: those notes are unstamped, would be read as
-- drifted, and would be retracted on that basis rather than on evidence.
--
-- It also made the telemetry lie. The run summary would have reported `fact_drift: 300`
-- on a night when nothing drifted, which is precisely the kind of fabricated signal that
-- caused `api_circuit_breakers.last_success_at` to be misread as a four-month outage.
--
-- So the two states are now separate:
--   fact_unstamped — no stamp yet. ELIGIBLE (so it acquires one), NEVER retractable.
--   fact_drift     — stamped, and the stamp disagrees with the country's current facts.
--                    ELIGIBLE and retractable, because that IS evidence.
--
-- Verified on prod at apply time: run_city_safety_backfill(3, true) returned
-- {examined:3, published:3, retracted:0, fact_drift:0, fact_unstamped:3} — unstamped rows
-- stamp themselves without retraction. A rolled-back mutation probe (Latvia set
-- criminalising) then returned drift=t, unstamped=f, confirming real drift IS caught while
-- the old country-name detector would not have fired.
