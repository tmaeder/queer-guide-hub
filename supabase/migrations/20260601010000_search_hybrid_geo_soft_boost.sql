-- search_hybrid relevance: in-query city extraction + soft city boost.
--
-- Problem (found via shadow vs Meili): the keyword leg uses websearch_to_tsquery
-- = AND. Multi-token queries like "gay hotel mykonos" require all tokens in one
-- doc (only 1 matched), so the common tokens (gay/hotel) dominate and the
-- location token is lost — PG returned all-Sydney, Meili surfaced Mykonos. The
-- worker's rewrite only extracts a city for non-EN or <3-word queries, so 3-word
-- English geo queries got no city signal on the PG path.
--
-- Fix: detect a city token in the query (exact match vs the cities index, guarded
-- to cities with >=10 venues so common nouns like "Bar" don't false-trigger),
-- strip it from the keyword query so the remaining terms drive topical match, and
-- add a SOFT +0.05 score boost for docs in that city. Soft (not a hard filter) so
-- cross-boundary results still appear below the in-city ones — matching Meili's
-- behaviour. Partial index on lower(city) keeps city detection cheap.
--
-- Validated (keyword-only): golden set all pass, zero-hit probe 0 failures,
-- known-item venue Recall@10 0.850 (= prior, MRR slightly better), latency
-- 254-712ms (< 1500ms gate). "gay hotel mykonos" -> Mykonos hotel #1 then
-- cross-boundary fallback; "lesbian bar berlin" -> all Berlin; non-geo queries
-- ("drag") byte-identical; no false-positive city ("leather bar").

create index if not exists search_documents_lower_city_venue
  on public.search_documents (lower(city)) where entity_type='venue';
...
qcity as (
  select cname from (
    select c.title as cname,
      (select count(*) from public.search_documents v where v.entity_type='venue' and lower(v.city)=lower(c.title)) as n
    from public.search_documents c, toks
    where c.entity_type='city' and c.title is not null and length(c.title) >= 4
      and lower(unaccent(c.title)) = any(toks.arr)
  ) z where z.n >= 10 order by z.n desc limit 1
),
params as (
  select nullif(btrim(p_query),'') as q0, (select cname from qcity) as dcity,
         case when p_lat is not null and p_lng is not null then st_setsrid(st_makepoint(p_lng,p_lat),4326)::geography end as origin
),
p2 as (
  select q0, dcity, origin, lower(coalesce(p_filters->>'city', dcity)) as boost_city,
    nullif(btrim(case when dcity is not null then regexp_replace(q0, dcity, '', 'gi') else q0 end),'') as eff_q
  from params
),
...
cand as (
  select sd.*, p.boost_city,
    case when p.eff_q is not null then ts_rank_cd(sd.search_tsv, websearch_to_tsquery('simple', unaccent(p.eff_q))) else 0 end as kw_rank,
    case when p.eff_q is not null then similarity(coalesce(sd.title,''), p.eff_q) else 0 end as trg,
  ...
  where (p_content_types is null or sd.entity_type = any(p_content_types))
    ...
    and (p.eff_q is null
         or sd.search_tsv @@ websearch_to_tsquery('simple', unaccent(p.eff_q))
         or similarity(coalesce(sd.title,''), p.eff_q) > 0.2
...
fused as (select c.*, coalesce(1.0/(60+kw.rk),0)+coalesce(1.0/(60+vec.rk),0) as rrf from cand c left join kw using(doc_id) left join vec using(doc_id)),
scored as (
  select f.*, f.rrf
    ...
    - case when f.dist_m is not null then least(f.dist_m/50000.0,1)*0.02 else 0 end as score
  from fused f
)
...
drop function if exists public.search_hybrid_geo(text, vector, text[], jsonb, double precision, double precision, double precision, timestamptz, integer, integer);
