-- Fix: a death_date in the FUTURE was treated as "recently deceased" (<=90d) and
-- wrongly assigned the 7-day refresh TTL instead of the 365-day default. Bound the
-- window on both sides so only genuinely recent (past) deaths get the 7-day cadence.
-- Mirrors the TS guard in _shared/personality-enrich-core.ts (age >= 0 && age <= 90).
...
SELECT
  ...
  CASE
    ...
    WHEN p.death_date IS NOT NULL AND p.death_date BETWEEN (now()::date - 90) AND now()::date THEN 7
    ELSE 365
  END                                                        AS ttl_days,
  (p.last_refreshed_at IS NULL
    OR p.last_refreshed_at < now() - (
      CASE
        ...
        WHEN p.death_date IS NOT NULL AND p.death_date BETWEEN (now()::date - 90) AND now()::date THEN interval '7 days'
