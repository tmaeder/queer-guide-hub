...
select set_config('app.actor', 'migration:60000101160100_styleguide_drift_vibrant_sweep', true);
    ...
    || jsonb_build_object(
         ...
         coalesce(c.field_provenance -> 'description', '{}'::jsonb)
           || jsonb_build_object(
                ...
                jsonb_build_object(
                  'from', t.before_txt,
                  'by',   'migration:60000101160100_styleguide_drift_vibrant_sweep',
                  'at',   now(),
...
BEGIN
  ...
  SELECT count(*) INTO v_fixed FROM public.cities
   WHERE field_provenance -> 'description' -> 'styleguide_corrected' ->> 'by'
         = 'migration:60000101160100_styleguide_drift_vibrant_sweep';
  IF v_fixed < 100 THEN
