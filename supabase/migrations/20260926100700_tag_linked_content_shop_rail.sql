-- get_tag_linked_content: Shop-rail hardening (finer-categorisation program, PR 6).
-- Body is the LIVE prod definition (md5 f3078a6c5d79bc84325fa1e6432a951c,
-- read back 2026-08-23) with exactly three changes to the marketplace section:
--   1. content_rating gate — tag pages are un-gated; the rail was serving
--      adult listings to anon (confirmed live: no rating filter existed).
--   2. + m.department in the payload — powers the department-aware
--      "Shop all" link on /tags/:slug.
--   3. ORDER BY boutique_score (was quality_score) — matches default browse.

CREATE OR REPLACE FUNCTION public.get_tag_linked_content(p_tag_id uuid, p_tag_name text, p_tag_slug text, p_limit integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
    ...
    'marketplace', COALESCE((
      ...
      FROM (
        SELECT m.id, m.title, m.brand, m.business_name, m.category,
               m.department, m.price, m.price_usd, m.currency,
        ...
        WHERE uta.tag_id = p_tag_id
          ...
          -- Tag pages are UN-GATED (no 18+ opt-in exists here): the Shop rail
          -- must never surface adult/explicit listings to anon readers —
          -- load-bearing now that concept auto-tagging multiplies rail content.
          AND COALESCE(m.content_rating, 'sfw') IN ('sfw', 'suggestive')
        ORDER BY m.featured DESC NULLS LAST, m.boutique_score DESC NULLS LAST
        LIMIT p_limit
      ) m_row
    ), '[]'::jsonb),
