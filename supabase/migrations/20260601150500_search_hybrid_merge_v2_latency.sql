-- Merge the search_hybrid_v2 latency rewrite into the served search_hybrid (v1).
-- HNSW-only vector recall (no per-candidate brute-force vec_sim) + bounded,
-- index-driven keyword recall (gin tsv @@ OR gin_trgm %). Warm in-DB latency
-- ~2000ms -> ~150ms (p95 RPC ~5.6s -> ~1.1s in shadow). Also folds in the modest
-- relevance gains validated in shadow (keyword-dominant weighted RRF 1.3/0.7 +
-- wrong-city penalty): overlap@10 vs Meili median 2->3, total-divergence 43%->27%.
-- Output shape + signature unchanged. search_hybrid_v2 continues as the candidate
-- for the in-progress LGBTQ-relevance work.
