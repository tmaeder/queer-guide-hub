-- News Quality — Phase 2: gate commit on the quality verdict.
--
-- The DAG commits ~500/run but quality-enhance does ~24/run, so most articles
-- committed BEFORE getting a quality_status/relevance/geo verdict — leaving live
-- rows with a NULL verdict. This holds rows that have been enriched but not yet
-- quality-enhanced so the next quality-enhance tick can verdict them first.
--
-- Escape hatch: only holds rows younger than 2h. If quality-enhance can't catch
-- up within 2h (circuit open, overload), the row commits anyway — nothing stalls
-- permanently. Rows that never entered enrichment (enrichment_status <> 'enriched')
-- are unaffected and commit as before.
--
-- Only the FOR-loop selection changed vs the prior definition.
  ...
  FOR v_row IN
    SELECT id, normalized_data, enriched_data, raw_data, source_name
     ...
     WHERE job_id = p_job_id AND target_table = 'news_articles' AND disposition = 'pending'
       ...
       -- Quality gate: hold enriched-but-not-yet-verdicted rows so quality-enhance
       -- can score them first. 2h escape hatch prevents permanent stalls.
       AND NOT (
