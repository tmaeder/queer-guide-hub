...
├── workers/
│   ├── mcp/              # CF Worker: remote MCP server (mcp.queer.guide) — McpAgent + OAuth
...
**MCP server (`mcp.queer.guide`, 2026-06-06):** Remote [Model Context Protocol](https://modelcontextprotocol.io) server in `workers/mcp/` — Cloudflare `agents` `McpAgent` (DO-backed) + `@cloudflare/workers-oauth-provider`. Re-wraps the existing discovery RPCs + the submit JWT path; almost no new business logic. Two mounts of one DO class: **public** `/mcp` (Streamable HTTP) + `/sse` — anonymous read tools (`search`, `autocomplete`, `get_entity`, `find_related`, `recommendations`, `events_in_window`, `on_this_day`, `knowledge`/AutoRAG); **authenticated** `/authed` behind OAuth 2.1 — adds write tools (`submit_place`, `submit_event`, `save_favorite`, `list_favorites`, `create_trip`, `list_my_trips`, `add_to_trip`). MCP **resources** `queerguide://city/{slug}` + `queerguide://country/{slug}` (pinnable destination context). Per-IP rate limit (240/60s, fail-open) via `[[ratelimits]]` binding. Reads use the **anon** key (all discovery RPCs granted to `anon` — no service-role key in a public worker); writes forward the user's Supabase JWT so RLS authorizes them. OAuth login is email-OTP against GoTrue → session stored in grant props. New RPC `get_entity_detail` (migration `20260606120000`). KV `OAUTH_KV` for grants/tokens. Write tools also registered on the public mount but return an auth hint until connected via `/authed`.
