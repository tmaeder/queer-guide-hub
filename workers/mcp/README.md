...
- MCP **resources** (city/country guide pages as pinnable context).
- Admin/pipeline tools (triage, dedup, workflow control).
- Marketplace checkout. Refreshed access tokens are not yet persisted back to the OAuth grant (refreshed in-memory per session).
