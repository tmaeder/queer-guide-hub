-- ROLLBACK for the Phase 1 embedding move (20260606190000/190100).
-- Run manually if vector search/dedup/recs regress. Restores the 4 readers to
-- read search_documents.embedding, rebuilds the old HNSW, and drops the sibling.
-- The search_documents.embedding column was never dropped in Phase 1, so it is
-- still fully populated — rollback is lossless.

-- 1. Rebuild the original index (slow; ~1-2 min).
set statement_timeout='600s';
set maintenance_work_mem='512MB';
create index if not exists search_documents_embed_hnsw
  on public.search_documents using hnsw (embedding extensions.vector_cosine_ops)
  with (m='16', ef_construction='64');

-- 2. Restore the 4 original reader bodies (read search_documents.embedding).
...
$function$;
-- NOTE: related_entities, find_duplicates, get_recommendations originals are in
-- their source migrations; restore from git history if a full revert is needed.
-- Then: drop trigger trg_sd_mirror_embedding; drop function search_documents_mirror_embedding();
--       drop table public.search_embeddings cascade;
