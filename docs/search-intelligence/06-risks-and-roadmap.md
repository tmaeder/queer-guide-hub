...
| **AI suggestion *producer* cutover (remaining producers)** | `auto-tag-content`, `automation-auto-tagger`, and `content-automation/modules/auto-tagger.ts` shipped above. Remaining direct-tag-write path: legacy `fetch-news` direct-upsert at `supabase/functions/fetch-news/index.ts:595-609` is **still live in production** despite CLAUDE.md's "old `fetch-news` direct-upsert is disabled" note — verification (2026-04-30) found cron `fetch-news-every-2-hours` is still scheduled (migration `20250805172222`, never unscheduled) and `workflow_definitions` registers `fetch-news` with `is_enabled=true`. The new `wf-news-pipeline` cron CLAUDE.md describes is referenced in code comments but not actually scheduled. Cutover requires (1) scheduling `wf-news-pipeline`, (2) unscheduling `fetch-news-every-2-hours`, (3) disabling the `fetch-news` workflow_definitions row, then (4) removing the direct-upsert. Sequence is operator-driven; not safe to do from a code PR. |
