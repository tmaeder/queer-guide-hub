...
The homepage city cards carry an octilinear abstraction of that city's **real**
rapid-transit network — Berlin's U-Bahn, Lisbon's Metro, Melbourne's trams —
instead of a decorative squiggle. `CityNetwork.tsx` renders it;
`cityNetworkGeometry.ts` is generated by
`scripts/generate-city-transit-lines.mjs` from OpenStreetMap route relations
and committed, so nothing is fetched at runtime and every diagram is reviewed
once by eye.
