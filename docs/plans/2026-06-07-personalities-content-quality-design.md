...
- **Connection backfill — five bugs found & fixed to make it actually flow.** Getting `lgbti_connection` to populate (from "unclear for everyone" → 672+ sourced connections and climbing) surfaced a cascade, each fixed: (1) **SKIP sentinels** — `wikidata_qid='SKIP_<uuid>'` fake anchors defeating the outing guard (now require real `Q[0-9]+`); (2) **rate-limit lock-in** — a heavy drain made source-API fetches fail, deriving nothing yet stamping rows done (`fetchWikipediaCategories` now returns null on failure; loop skips stamping → retries); (3) **view-multiplier priority** — `debt × ln(views)` starved low-traffic icons (→ debt-dominant); (4) **birth-name duplicates** — "Farrokh Bulsara"→Freddie Mercury etc. resolved to an already-used QID, failed the UNIQUE update, and clogged the queue head forever (→ auto-link as `duplicate_of` on collision); (5) **adult cohort dominating the debt tier** — no-Wikipedia adult rows yielding nothing ahead of icons (→ non-adult-first priority). Verified: Owen Jones → activist, 672 connections (538 community_member + 134 activist), all sourced via Wikidata P91/P21 + Wikipedia BLPCAT categories. Drive edge functions calling external APIs GENTLY (paced) or via the 30-min cron.
- **Follow-up SHIPPED — Wikipedia-category connection + SKIP-sentinel fix.** Extended the connection derivation to **Wikipedia categories** (far higher coverage than Wikidata P91/P21; identity categories are WP:BLPCAT-sourced): activist categories → `activist`, identity categories → `community_member`, ally/opponent/icon skipped. Combined with the P91/P21 signal. **While verifying, found a real bug:** ~1,329 "anchored" rows carried a `wikidata_qid` of `SKIP_<uuid>` (a "no match, don't retry" sentinel for adult performers) — fake anchors that inflated the metric (real Q ~3,595 not ~4,925), **defeated the outing guard** (its provenance check only tested NOT NULL), and made the loop 404. Fixed: loop respects SKIP; outing guard (trigger + gate) now requires a real Q-number or non-SKIP source; metrics count real Q only (honest **37.6%**, was an inflated 51.5%) and surface `skip_sentinel` as its own bucket. The gate re-merge also restored the Phase 0 strengthening a concurrent change had reverted. All 5 critical gates green. Verified: Pol Prince/Jay Wu → community_member after the fix.
