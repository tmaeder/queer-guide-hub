...
- **External checks:** `WebFetch` URL-liveness on a **sample of 6/25** hotline URLs + web spot-corroboration of the 7 death-penalty travel records. Phone numbers **cannot be dialed** — validated for format/region only, **not liveness-tested**.
...
|---|---|---|
| **help-hotlines** | **55** | Numbers accurate, but **0 / 25 carry any `verified_at`** — the vetting promise is metadata-unbacked on the highest-stakes surface. |
...
2. **`hotline_unverified` + `hotline_reachable` + `hotline_url_live`** — a 90-day verification gate on all 25 crisis records plus reachability/TLS checks. Turns the vetting promise into enforced fact on the highest-stakes page.
