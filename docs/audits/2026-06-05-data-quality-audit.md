...
**🟢 News full-text backfill — RUNNING.** `scripts/backfill-news-fulltext.mjs` under `scripts/run-supervised.sh` (detached, indefinite-retry backoff, exit-42 disk-halt). Re-fetches the URL of each thin live article (`content < 120` chars, ~7,658) and recovers the body via a **faithful jsdom port of the shipped `_shared/news-quality/extract.ts`** (JSON-LD `articleBody` → `<article>` → `<main>` → densest `<p>`-cluster). Conservative swap (≥250 chars AND ≥1.2× current), never blanks on 404/paywall/non-HTML. Steady yield ~63% extracted, 0 failures, ~25 articles/min (~5h ETA). Resumable via id-keyset cursor (`scripts/output/news-fulltext.cursor`); disk-guarded (hard-stop at 6,300 MB; flat at 5,794 MB after first ~300 — content adds are tiny vs DB size). Writes `content` + `excerpt`.
...
- **Tagged live news: 27 → 337.** Remaining **16,415** empty → **deferred to `pipeline-enrich-news` LLM geo step** (needs body comprehension + relevance, not keyword matching). Reversal if ever needed: clear `country_ids` on rows where the single tag matches the matcher and `updated_at` falls in the apply window.
