...
5. **RLS for RPCs.** `SECURITY DEFINER` RPCs are granted to `anon` — safe for read; inputs sanitized by `search_path` pin. `track_user_event` should additionally validate `user_id` matches JWT `sub` when called from browser — add via a wrapper with `auth.uid()`.
