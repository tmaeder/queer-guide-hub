...
export function useAutomationMonitor() {
  ...
  const { data: flagStats } = useQuery({
    ...
    queryFn: async () => {
      const [pending, applied, rejected] = await Promise.all([
        supabase.from('content_flags' as never).select('*', { count: 'exact', head: true }).eq('status', 'pending'),
        supabase.from('content_flags' as never).select('*', { count: 'exact', head: true }).eq('status', 'applied'),
        supabase.from('content_flags' as never).select('*', { count: 'exact', head: true }).eq('status', 'rejected'),
      ]);
      return {
  ...
  const updateModuleConfig = useMutation({
    mutationFn: async ({ moduleId, updates }: { moduleId: string; updates: Partial<AutomationModule> }) => {
      const { error } = await supabase
        .from('automation_modules' as never)
