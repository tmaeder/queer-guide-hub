...
export function useGranularRoles(): GranularRolesReturn {
  ...
  // Permission check
  const can = useCallback((permission: Permission, contentType?: string): boolean => {
  ...
  const canAccess = useCallback((section: 'cockpit' | 'content' | 'import-review' | 'system'): boolean => {
    if (isAdmin) return true;
    ...
    switch (section) {
      ...
      case 'import-review':
        return isAdmin || isModerator || permissions.some((p) => p.permissions.includes('review') || p.permissions.includes('import'));
      case 'system':
        return isAdmin;
      default:
        return false;
