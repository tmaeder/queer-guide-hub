...
function scoreCompleteness(data: Record<string, unknown>, contentType: string): { score: number; details: QualityDetail[] } {
  ...
  const optionalFilled = allFields.filter(f => !required.includes(f) && data[f] != null && data[f] !== '').length;
...
export function useQualityGates(thresholds?: Partial<QualityThresholds>) {
  ...
  const scoreItem = useCallback((data: Record<string, unknown>, contentType: string): QualityScore => {
  ...
  const getDecision = useCallback((score: QualityScore): QualityDecision => {
    if (score.overall >= t.autoApprove) return 'auto_approve';
    if (score.overall >= t.review) return 'review';
    return 'reject';
  }, [t]);

  const applyQualityGate = useCallback(async (
    stagingId: string,
