...
async function fetchSystemHealth(): Promise<SystemHealth> {
  ...
  const { error } = await supabase.from('venues' as any).select('id', { count: 'exact', head: true }).limit(1);
...
async function fetchReviewSummary(): Promise<ReviewSummary> {
  const [stagingRes, cmsRes, modRes, autoRes, tagRes] = await Promise.all([
    supabase.from('ingestion_staging' as any).select('id', { count: 'exact', head: true }).eq('review_status', 'pending_review').eq('disposition', 'pending'),
    supabase.from('cms_content_metadata' as any).select('id', { count: 'exact', head: true }).eq('workflow_state', 'review'),
    supabase.from('moderation_flags' as any).select('id', { count: 'exact', head: true }).eq('status', 'OPEN'),
    supabase.from('content_flags' as any).select('id', { count: 'exact', head: true }).eq('status', 'pending'),
    supabase.from('tag_suggestions' as any).select('id', { count: 'exact', head: true }).eq('status', 'pending'),
  ]);
...
async function fetchImportSummary(): Promise<ImportSummary> {
  ...
  const [activeRes, completedRes, failedRes] = await Promise.all([
    supabase.from('import_jobs' as any).select('id', { count: 'exact', head: true }).in('status', ['processing', 'validating', 'pending']),
    supabase.from('import_jobs' as any).select('id', { count: 'exact', head: true }).eq('status', 'completed').gte('completed_at', todayISO),
    supabase.from('import_jobs' as any).select('id', { count: 'exact', head: true }).eq('status', 'failed').gte('created_at', todayISO),
  ]);
...
async function fetchQualityIndex(): Promise<QualityIndex> {
  const [warningRes, criticalRes] = await Promise.all([
    supabase.from('content_flags' as any).select('id', { count: 'exact', head: true }).eq('status', 'pending').in('severity', ['warning', 'info']),
    supabase.from('content_flags' as any).select('id', { count: 'exact', head: true }).eq('status', 'pending').in('severity', ['critical', 'error']),
  ]);
