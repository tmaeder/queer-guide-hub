...
export const SINGLE_SPINE = [
  {
    id: 'breadcrumb',
    label: 'Breadcrumb',
    note: 'Line, then parent, then this page. Three levels or fewer.',
  },
  {
    id: 'bullet',
    label: 'Bullet and kicker',
    note: "Route bullet in the type's colour, plus its track.",
  },
  {
    id: 'title',
    label: 'Title and standfirst',
    note: 'Anton title, one sentence of what this is.',
  },
  ...
  { id: 'provenance', label: 'Provenance', note: 'Who added it, when checked, how to correct it.' },
  {
    ...
    note: 'Related items as a bending line, typed by bullet.',
  },
...
export const SINGLE_MODULES = [
  {
    ...
    label: 'Fact strip',
    slot: 'head',
    carries: 'when, where, cost, access',
  },
  { n: 2, id: 'hours', label: 'Hours table', slot: 'body', carries: 'opening hours, holidays' },
  {
    n: 3,
    id: 'occurrences',
    label: 'Occurrence board',
    slot: 'body',
    carries: 'dates, times, status',
  },
  ...
  { n: 7, id: 'roster', label: 'Roster', slot: 'body', carries: 'people, roles' },
  {
    n: 8,
    id: 'nested-entity',
    label: 'Nested entity card',
    slot: 'body',
    carries: 'linked entity',
  },
...
> = {
  ...
  event: {
    label: 'Events',
    required: [1, 3, 4, 8, 15],
    conditional: [7, 11, 16, 9],
    owner: 'Occurrence board',
  },
  country: {
    label: 'Countries',
    required: [1, 12, 16, 11],
  ...
  // MODULE 09 (Variant picker) is RENDERABLE since the 2026-08
  // finer-categorisation program: `marketplace_listing_variants` carries
  // per-variant size/colour/price/stock and `VariantAvailability`
  // (src/components/marketplace/VariantAvailability.tsx) owns the module.
  // It stays self-hiding for listings with no variant rows (rule 2), and the
  // chips are non-interactive — there is no cart; purchase completes on the
  // merchant's site, so "offered sizes" is a statement, not a selector.
  marketplace: {
    label: 'Marketplace',
    required: [1, 9, 15, 8],
    conditional: [11, 12, 4],
    owner: 'Variant picker',
  },
  organization: {
    label: 'Business',
    required: [1, 2, 4, 8, 11],
    conditional: [16, 3, 15, 7],
    owner: 'Nested entity card',
  },
  tag: {
    label: 'Tags (wiki)',
    required: [1, 12, 7],
