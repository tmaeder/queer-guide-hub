...
describe('SubmitForm', () => {
  ...
  it('derives the content type from the path when there is no :contentType param', () => {
    // The explicit per-type submit/<slug> routes (routes.tsx) carry no :contentType
    // param, so SubmitForm reads the last path segment. An unknown segment still
    // resolves to the fallback rather than crashing on an undefined type.
    const { getByText } = render(
      <MemoryRouter initialEntries={['/submit/zzinvalid']}>
        <Routes>
