...
vi.mock('@/hooks/useVenues', () => ({
  useVenues: () => ({ createVenue: vi.fn(), updateVenue: vi.fn(), deleteVenue: vi.fn(), refetch: vi.fn() }),
}));
vi.mock('@/hooks/useAddressResolver', () => ({ useAddressResolver: () => ({ resolveAddress: vi.fn() }) }));
vi.mock('./admin-venues/VenueColumns', () => ({ useVenueColumns: () => [] }));
...
describe('AdminVenues', () => {
  it('renders without crashing', () => {
    const { container } = render(<AdminVenues />, { wrapper });
