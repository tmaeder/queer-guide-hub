...
import { Plus, Search, Edit, Trash2, ArrowLeft, MapPin, Building } from 'lucide-react';
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setFormData((prev) => ({ ...prev, is_capital: checked as boolean }))
                      }
                    />
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setFormData((prev) => ({ ...prev, is_major_city: checked as boolean }))
                      }
                    />
        ...
        <Card>
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <div>
                <Typography sx={{ fontSize: '1.5rem', fontWeight: 700 }}>
                  {cities.filter((c) => c.is_capital).length}
                </Typography>
                <Typography sx={{ fontSize: '0.875rem', color: 'text.secondary' }}>
                  Capital Cities
                </Typography>
        ...
        <Card>
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <div>
                <Typography sx={{ fontSize: '1.5rem', fontWeight: 700 }}>
                  {cities.filter((c) => c.is_major_city).length}
                </Typography>
                <Typography sx={{ fontSize: '0.875rem', color: 'text.secondary' }}>
                  Major Cities
                </Typography>
            ...
            {filteredCities.map((city) => (
              ...
              >
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    <div>Country: {countries.find((c) => c.id === city.country_id)?.name}</div>
                    {city.region_name && <div>Region: {city.region_name}</div>}
                    {city.population && <div>Population: {city.population.toLocaleString()}</div>}
                  </Box>
                </Box>
