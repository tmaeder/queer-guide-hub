...
vi.mock('@/integrations/supabase/client', () => { const h: ProxyHandler<any> = { get: (_t, p) => { if (p === 'then') return undefined; return (..._a: any[]) => new Proxy(() => {}, h); }, apply: () => new Proxy(() => {}, h) }; return { supabase: { from: () => new Proxy(() => {}, h), channel: () => new Proxy(() => {}, h), removeChannel: vi.fn() } }; });
