...
export function AutomationDashboard() {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {/* Stats Overview */}
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(4, 1fr)', lg: 'repeat(7, 1fr)' }, gap: 2 }}>
        <StatCard icon={Zap} label="Active Modules" value={stats.enabledModules} total={stats.totalModules} />
        <StatCard icon={AlertTriangle} label="Pending Review" value={stats.pendingFlags} color={stats.pendingFlags > 0 ? '#f59e0b' : undefined} />
        <StatCard icon={CheckCircle2} label="Applied" value={stats.appliedFlags} color="#22c55e" />
        <StatCard icon={Link2} label="Dead Links" value={stats.deadLinks} color={stats.deadLinks > 0 ? '#ef4444' : undefined} />
        <StatCard icon={MapPin} label="Geo Mismatches" value={stats.geoMismatches} color={stats.geoMismatches > 0 ? '#f59e0b' : undefined} />
        ...
        <TabsList>
          ...
          <TabsTrigger value="review">
            ...
            {stats.pendingFlags > 0 && (
              <Badge variant="destructive" style={{ marginLeft: 6, fontSize: '0.65rem', padding: '0 4px' }}>
                {stats.pendingFlags}
              </Badge>
            )}
          </TabsTrigger>
                ...
                <Card key={mod.id}>
                  <CardContent sx={{ py: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      {/* Icon + Info */}
                      <Box sx={{ p: 1, bgcolor: mod.is_enabled ? '#dcfce7' : '#f3f4f6', borderRadius: 2, display: 'flex' }}>
                        <CategoryIcon size={20} style={{ color: mod.is_enabled ? '#16a34a' : '#9ca3af' }} />
                      </Box>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Typography sx={{ fontWeight: 600, fontSize: '0.95rem' }}>{mod.display_name}</Typography>
                          {mod.schedule && (
                            <Chip size="small" label={mod.schedule} variant="outlined" sx={{ fontSize: '0.7rem', height: 20 }} />
                          )}
                          {mod.auto_approve && (
                            <Chip size="small" label="Auto-approve" color="success" sx={{ fontSize: '0.7rem', height: 20 }} />
                          )}
                        </Box>
                        <Typography sx={{ fontSize: '0.8rem', color: 'text.secondary', mt: 0.25 }}>
                          {mod.description}
                        </Typography>
                        <Box sx={{ display: 'flex', gap: 2, mt: 0.5, fontSize: '0.75rem', color: 'text.secondary' }}>
                          ...
                          {mod.last_run_at && (
                            <span>Last run: {formatDistanceToNow(new Date(mod.last_run_at), { addSuffix: true })}</span>
                          )}
                          {mod.last_run_status && (
                            <Chip
                              size="small"
                              label={mod.last_run_status}
                              color={mod.last_run_status === 'success' ? 'success' : mod.last_run_status === 'partial' ? 'warning' : 'error'}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Tooltip title={mod.is_enabled ? 'Disable' : 'Enable'}>
                          <Switch
                            ...
                            disabled={isToggling}
                            onChange={() => toggleModule({ moduleId: mod.id, enabled: !mod.is_enabled })}
                          />
                        </Tooltip>
                      </Box>
                    </Box>
            ...
            <CardHeader>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box>
                  ...
                  <CardDescription>
                    {stats.pendingFlags} pending &middot; {stats.appliedFlags} applied &middot; {flagStats?.rejected || 0} rejected
                  </CardDescription>
                </Box>
              </Box>
            </CardHeader>
                    ...
                    <TableBody>
                      {pendingFlags.map(flag => (
                        <TableRow key={flag.id} hover>
                          ...
                          <TableCell>
                            <Typography
                              sx={{ fontSize: '0.85rem', fontWeight: 500, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}
                            ...
                            {flag.description && (
                              <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary', mt: 0.25 }}>
                                {flag.description.slice(0, 80)}{flag.description.length > 80 ? '...' : ''}
                              </Typography>
                            )}
                          </TableCell>
                          <TableCell>
                            <Chip size="small" label={flag.content_type} variant="outlined" sx={{ fontSize: '0.65rem' }} />
                          </TableCell>
                          <TableCell>
                            <Chip
                              size="small"
                                ...
                                <Tooltip title="Approve & Apply">
                                  <span>
                                    <IconButton
                                      ...
                                      onClick={() => reviewFlag({ flagId: flag.id, action: 'approved' })}
                                    >
                                      <ThumbsUp size={14} />
                                    </IconButton>
                                  </span>
                              ...
                              <Tooltip title="Reject">
                                <span>
                                  <IconButton
                                    ...
                                    color="error"
                                    disabled={isReviewing}
                                    onClick={() => reviewFlag({ flagId: flag.id, action: 'rejected' })}
                                  >
                                    <ThumbsDown size={14} />
                                  </IconButton>
                                </span>
                    ...
                    <TableBody>
                      {deadLinks.map(link => (
                        <TableRow key={link.id} hover>
                          <TableCell>
                            <Chip size="small" label={link.content_type} variant="outlined" sx={{ fontSize: '0.65rem' }} />
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <Typography
                              sx={{ fontSize: '0.75rem', fontFamily: 'monospace', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                          ...
                          <TableCell>
                            <Typography sx={{ fontSize: '0.75rem', color: 'error.main', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {link.error_message || '—'}
                            </Typography>
                          </TableCell>
                          <TableCell>
                    ...
                    <TableBody>
                      {geoMismatches.map(geo => (
                        <TableRow key={geo.id} hover>
                          <TableCell>
                            <Chip size="small" label={geo.content_type} variant="outlined" sx={{ fontSize: '0.65rem' }} />
                          ...
                          <TableCell>
                            <Typography sx={{ fontSize: '0.75rem', color: 'warning.main', maxWidth: 250 }}>
                          ...
                          <TableCell>
                            <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
                              {formatDistanceToNow(new Date(geo.last_validated_at), { addSuffix: true })}
                            </Typography>
                          </TableCell>
                        </TableRow>
                      ))}
      ...
      <FlagDetailDialog
        ...
        onApprove={(id) => { reviewFlag({ flagId: id, action: 'approved' }); setSelectedFlag(null); }}
        onReject={(id) => { reviewFlag({ flagId: id, action: 'rejected' }); setSelectedFlag(null); }}
      ...
      <ModuleConfigDialog
        ...
        onSave={(id, updates) => { updateModuleConfig({ moduleId: id, updates }); setConfigModule(null); }}
      />
    </Box>
  );
}
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            ...
            <Chip size="small" label={flag.severity} color={SEVERITY_COLORS[flag.severity] || 'default'} />
            ...
            {flag.confidence != null && (
              <Chip size="small" label={`Confidence: ${(flag.confidence * 100).toFixed(0)}%`} variant="outlined" />
            )}
          </Box>
          ...
          {flag.current_value && (
            <Box>
              <Typography sx={{ fontWeight: 600, fontSize: '0.85rem', mb: 0.5 }}>Current Value</Typography>
              <Box sx={{ p: 1.5, bgcolor: '#fef2f2', borderRadius: 1, fontFamily: 'monospace', fontSize: '0.8rem', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
          ...
          {flag.suggested_value && (
            <Box>
              <Typography sx={{ fontWeight: 600, fontSize: '0.85rem', mb: 0.5 }}>Suggested Value</Typography>
              <Box sx={{ p: 1.5, bgcolor: '#f0fdf4', borderRadius: 1, fontFamily: 'monospace', fontSize: '0.8rem', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
          ...
          <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
            Content ID: {flag.content_id} &middot; Created {formatDistanceToNow(new Date(flag.created_at), { addSuffix: true })}
          </Typography>
        </Box>
      </DialogContent>
      <DialogActions>
