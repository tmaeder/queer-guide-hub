  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-background shrink-0 z-10">
      ...
      >
        ...
        <SelectContent>
          <SelectItem value="__new__" className="text-xs italic text-muted-foreground">
            — New pipeline —
          </SelectItem>
      ...
      <Input
        ...
        onChange={(e) => {
          p.setPipelineName(e.target.value);
          p.setIsDirty(true);
        }}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              onClick={() => {
                if (!window.confirm('Discard all unsaved changes and reload from saved version?'))
                  return;
      ...
      <Button
        size="sm"
        variant="outline"
        onClick={() => p.handleRun({ dryRun: true })}
      ...
      <Tooltip>
        ...
        <TooltipContent className="text-xs">
          Undo <span className="ml-1 text-muted-foreground">{isMac ? '⌘Z' : 'Ctrl+Z'}</span>
        </TooltipContent>
      </Tooltip>
      <Tooltip>
        <TooltipTrigger asChild>
        ...
        <TooltipContent className="text-xs">
          Redo <span className="ml-1 text-muted-foreground">{isMac ? '⌘⇧Z' : 'Ctrl+Y'}</span>
        </TooltipContent>
      </Tooltip>
      <Tooltip>
        <TooltipTrigger asChild>
        ...
        <TooltipContent className="text-xs">
          Auto-layout <span className="ml-1 text-muted-foreground">{isMac ? '⌘L' : 'Ctrl+L'}</span>
        </TooltipContent>
      </Tooltip>
      <Tooltip>
        <TooltipTrigger asChild>
        ...
        <TooltipContent className="text-xs">
          Quick-add node{' '}
          <span className="ml-1 text-muted-foreground">{isMac ? '⌘K' : 'Ctrl+K'}</span>
        </TooltipContent>
      </Tooltip>
      <Tooltip>
        <TooltipTrigger asChild>
      ...
      />
      <AccessDialog pipelineId={p.selectedPipelineId} pipelineName={p.pipelineName} />
      ...
      />
      {p.nodeTypeList && (
        <AISuggestDialog nodeTypes={p.nodeTypeList} onApply={p.applyAISuggestion} />
      )}
      <ImportExportMenu
        nodes={p.nodes}
        edges={p.edges}
      ...
      {p.activeRunId &&
        p.runStatus &&
        ['completed', 'failed', 'cancelled'].includes(p.runStatus) && (
          <Button
            size="sm"
            variant="ghost"
            className="text-xs"
      ...
      <div className="ml-auto flex items-center gap-1 text-xs text-muted-foreground">
        ...
        {p.viewingRunId && (
          ...
          >
            ...
            viewing historical
            <button className="ml-1 hover:underline" onClick={() => p.setViewingRunId(null)}>
              ×
            </button>
          </Badge>
        )}
        {!p.viewingRunId && p.latestRun && !p.activeRunId && (
          ...
          >
            <Clock className="h-3 w-3" />
            {formatDistanceToNow(new Date(p.latestRun.started_at || p.latestRun.created_at), {
              addSuffix: true,
            })}{' '}
            • {p.latestRun.status}
          </Badge>
        ...
        {p.validationCount > 0 && (
          <Tooltip>
            ...
            <TooltipContent className="text-xs max-w-[240px]">
              {p.validationCount} node{p.validationCount === 1 ? '' : 's'} with missing required
              config. Click a node to see its config panel.
            </TooltipContent>
          </Tooltip>
        )}
        {p.isDirty && (
        ...
        <Button
          ...
          onClick={() => p.navigate('/admin/pipelines?tab=monitor')}
        >
        ...
        <Button
          ...
          variant="ghost"
          className="h-7 text-xs"
          onClick={() => p.navigate('/admin/imports')}
        >
