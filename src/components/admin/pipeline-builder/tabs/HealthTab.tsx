...
export default function HealthTab() {
  ...
  const { data: deadLetter = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const groups: Record<
        string,
        { stage: string; errorClass: string; count: number; sample: string }
      > = {};
      for (const r of (data ?? []) as unknown as Array<{
        ...
        const errMsg = String(
          (r.payload as Record<string, unknown>)?.error ??
            (r.payload as Record<string, unknown>)?.crash ??
            r.new_status,
        );
        const errorClass = errMsg.split(':')[0].slice(0, 60);
        const k = `${r.stage}::${errorClass}`;
        if (!groups[k])
          groups[k] = { stage: r.stage, errorClass, count: 0, sample: errMsg.slice(0, 200) };
        groups[k].count++;
      }
      ...
      <div className="flex flex-col gap-6">
        ...
        <SectionCard
          ...
          extra={
            <>
              ...
              {deadLetter.length === 0 && (
                <span className="text-xs2 font-normal text-muted-foreground">(no failures)</span>
              )}
            </>
          }
        >
          {deadLetter.length > 0 && (
            <div className="grid grid-cols-[120px_1fr_80px] gap-x-4 gap-y-1 text-xs">
              ...
              <div className="font-semibold text-muted-foreground uppercase tracking-wider text-2xs text-right">
                Count
              </div>
              {deadLetter.map((g, i) => (
                <div key={i} className="contents">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="py-1 border-t border-border/40 truncate cursor-help">
                        {g.errorClass}
                      </div>
                    </TooltipTrigger>
                  ...
                  <div
                    className={`py-1 border-t border-border/40 text-right font-mono font-semibold ${g.count > 10 ? 'text-destructive' : ''}`}
                  >
                    {g.count}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-[1fr_80px_80px_80px] gap-x-4 gap-y-1 text-xs">
              ...
              {enrichSummary.map((s, i) => (
                <div key={i} className="contents">
                  ...
                  <div className="py-1 border-t border-border/40 text-right tabular-nums">
                    {s.success}
                  </div>
                  <div
                  ...
                  <div
                    className={`py-1 border-t border-border/40 text-right tabular-nums ${s.failed > 0 ? 'text-destructive font-semibold' : ''}`}
                  >
                    {s.failed}
                  </div>
                </div>
              ...
              {circuitBreakers.map((cb) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span
                      className={`text-3xs font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${cbClass[cb.state] || 'bg-muted'}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-1 text-xs2 text-muted-foreground">
                    <div>
                      Fails:{' '}
                      <span
                        className={`font-mono ${cb.failure_count > 0 ? 'text-destructive font-semibold' : ''}`}
                      >
                        {cb.failure_count}/{cb.threshold}
                      </span>
                    </div>
                    <div>
                      OK: <span className="font-mono">{cb.success_count}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <SectionCard
          ...
          extra={
            (geoHealth?.geo_merge_candidates ?? 0) > 0 ? (
              <Badge
                variant="outline"
                className="text-2xs px-1.5 py-0 bg-primary/10 text-primary border-primary/30"
              >
                {geoHealth?.geo_merge_candidates} merge candidates
              </Badge>
            ) : undefined
          }
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                {queueMetrics.map((q) => (
                  ...
                  >
                    ...
                    <div className="flex gap-4 text-xs text-muted-foreground">
                      ...
                      <span>
                        Total: <span className="tabular-nums">{q.total_messages as number}</span>
                      </span>
                    </div>
                  </div>
                ))}
          ...
          <SectionCard
            ...
            extra={
              <Badge variant="outline" className="text-2xs px-1.5 py-0">
                {totalStaging.toLocaleString()} items
              </Badge>
            }
          >
                ...
                <div className="grid grid-cols-[repeat(auto-fit,minmax(100px,1fr))] gap-2">
                  {stagingStats.map((s) => (
                    ...
                    >
                      ...
                      <div className="text-2xs text-muted-foreground capitalize mt-0.5">
                        {s.status}
                      </div>
                    </div>
                  ))}
