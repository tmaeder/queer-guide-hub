...
function StatBlock({ label, value, variant }: { label: string; value: number; variant?: 'success' | 'warning' | 'destructive' | 'default' }) {
  const colorClass =
    variant === 'success' ? 'text-foreground dark:text-foreground' :
    variant === 'warning' ? 'text-foreground dark:text-foreground' :
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="border border-border rounded-element bg-background overflow-hidden">
        <div className="px-4 py-2 border-b border-border text-xs font-semibold text-muted-foreground flex items-center gap-2 flex-wrap">
          ...
          {paused > 0 && <Badge variant="outline" className="text-2xs px-1.5 py-0 bg-destructive/10 dark:bg-destructive/30 text-destructive dark:text-destructive border-destructive dark:border-destructive">{paused} auto-paused</Badge>}
          {errored > 0 && <Badge variant="outline" className="text-2xs px-1.5 py-0 bg-muted dark:bg-foreground/30 text-foreground dark:text-foreground border-border dark:border-border">{errored} errored</Badge>}
        </div>
        <div className="max-h-[400px] overflow-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 sticky top-0">
              <tr className="border-b border-border">
                <th className="text-left px-4 py-2 font-medium text-muted-foreground text-xs2 uppercase tracking-wider">Source</th>
                <th className="text-left px-4 py-2 font-medium text-muted-foreground text-xs2 uppercase tracking-wider w-[100px]">Status</th>
                <th className="text-right px-4 py-2 font-medium text-muted-foreground text-xs2 uppercase tracking-wider w-[80px]">Fails</th>
                <th className="text-right px-4 py-2 font-medium text-muted-foreground text-xs2 uppercase tracking-wider w-[100px]">Reliability</th>
                <th className="text-right px-4 py-2 font-medium text-muted-foreground text-xs2 uppercase tracking-wider w-[90px]">Avg/fetch</th>
              </tr>
            </thead>
            <tbody>
              {sources.length === 0 ? (
                ...
                return (
                  <tr key={String(s.id)} className="border-b border-border/40 hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-2 truncate max-w-[280px]" title={String(s.name)}>
                      {String(s.name)}
                      {s.auto_paused ? <Badge variant="outline" className="ml-2 text-3xs px-1 py-0 bg-destructive/10 dark:bg-destructive/30 text-destructive dark:text-destructive border-destructive dark:border-destructive">paused</Badge> : null}
                    </td>
                    <td className={`px-4 py-2 text-xs ${statusColor}`}>{status}</td>
                    <td className={`px-4 py-2 text-right font-mono tabular-nums ${failures > 0 ? 'text-destructive' : 'text-muted-foreground'}`}>
                      {failures}
                    </td>
                    <td className="px-4 py-2 text-right font-mono tabular-nums">{reliability.toFixed(2)}</td>
                    <td className="px-4 py-2 text-right font-mono tabular-nums text-muted-foreground">{avg.toFixed(1)}</td>
                  </tr>
                );
              })}
            </tbody>
