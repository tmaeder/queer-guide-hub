...
export default function VersionHistoryDialog({
  ...
  const { data: versions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await untypedFrom('pipeline_definition_versions')
        .select(
          'id, pipeline_id, version, name, display_name, description, nodes, edges, schedule, saved_by, saved_at',
        )
  ...
  const handleRevert = (v: VersionRow) => {
    ...
    if (
      !window.confirm(
        `Revert canvas to v${v.version}? Your current unsaved work will be replaced. You'll still need to Save to persist.`,
      )
    )
      return;
    onRevert(v);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Each save creates a snapshot. Revert to an older version to load it on the canvas (not
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge
                          variant="outline"
                          className={`text-2xs px-1.5 py-0 font-mono ${isCurrent ? 'bg-primary/10 text-primary border-primary/30' : ''}`}
                        >
                          v{v.version}
                        </Badge>
                        {isCurrent && (
                        ...
                        <span
                          ...
                          title={new Date(v.saved_at).toISOString()}
                        >
