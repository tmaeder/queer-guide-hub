...
const MODULES = [
  { value: 'ai-content-enhancer', label: 'AI Content Enhancement', description: 'Generate improved descriptions using AI' },
  { value: 'content-quality-checker', label: 'Quality Check', description: 'Find encoding issues, broken HTML, short descriptions' },
  { value: 'link-validator', label: 'Link Validation', description: 'Check for dead links and tracking parameters' },
  ...
  { value: 'contact-normalizer', label: 'Contact Normalization', description: 'Validate emails, URLs, and phone numbers' },
  { value: 'date-normalizer', label: 'Date Normalization', description: 'Detect past events and missing end times' },
];
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      ...
      <DialogContent style={{ maxWidth: 540 }}>
        <DialogHeader>
          ...
          <DialogDescription>
            Run automation modules to validate, enrich, and improve content in batch.
            Results appear in the Automation review queue.
          </DialogDescription>
        </DialogHeader>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, mt: 1 }}>
          ...
          <Box sx={{ bgcolor: 'action.hover', borderRadius: 1, p: 1.5 }}>
            <Typography variant="caption" color="text.secondary">
              The module will process items according to its configured batch size and create
              flags in the automation review queue. High-confidence changes may be auto-approved
          ...
          {result && !loading && (
            <Box sx={{
              ...
              {result.errors.length > 0 && (
                <Box sx={{ mt: 1, maxHeight: 100, overflow: 'auto', fontSize: '0.75rem', color: 'error.main' }}>
                  {result.errors.slice(0, 5).map((e, i) => (
                    <Typography key={i} variant="caption" sx={{ display: 'block', color: 'error.main' }}>
                      {e}
                    </Typography>
                  ))}
                </Box>
