...
const IMPORT_SOURCES: ImportSource[] = [
  { id: 'csv', label: 'CSV Upload', icon: FileSpreadsheet, description: 'Upload a CSV file with data', sourceType: 'csv', contentTypes: ['venues', 'events', 'personalities', 'cities', 'countries', 'hotels'] },
  { id: 'foursquare', label: 'Foursquare', icon: Globe, description: 'Import venues from Foursquare API', sourceType: 'api', contentTypes: ['venues'] },
  { id: 'google-places', label: 'Google Places', icon: Globe, description: 'Import venues from Google Places', sourceType: 'api', contentTypes: ['venues'] },
  { id: 'tripadvisor', label: 'TripAdvisor', icon: Globe, description: 'Import venues from TripAdvisor', sourceType: 'api', contentTypes: ['venues'] },
  { id: 'tomtom', label: 'TomTom', icon: Globe, description: 'Import venues from TomTom Places', sourceType: 'api', contentTypes: ['venues'] },
  { id: 'eventbrite', label: 'Eventbrite', icon: Globe, description: 'Import events from Eventbrite', sourceType: 'api', contentTypes: ['events'] },
  { id: 'ticketmaster', label: 'Ticketmaster', icon: Globe, description: 'Import events from Ticketmaster', sourceType: 'api', contentTypes: ['events'] },
  { id: 'rss', label: 'RSS Feed', icon: Rss, description: 'Import articles from RSS feeds', sourceType: 'scraping', contentTypes: ['news_articles'] },
];
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          {IMPORT_SOURCES.map((source) => {
            ...
            return (
              <Paper
                key={source.id}
                variant="outlined"
                onClick={() => setConfig({ ...config, source: source.id, contentType: source.contentTypes[0] })}
                sx={{
                  p: 2,
                  cursor: 'pointer',
                  textAlign: 'center',
              ...
              >
                <source.icon size={24} style={{ color: isSelected ? '#8b5cf6' : '#9ca3af', marginBottom: 4 }} />
                <Typography variant="body2" sx={{ fontWeight: isSelected ? 600 : 500, fontSize: '0.8rem' }}>
                  {source.label}
                </Typography>
                <Chip
      ...
      <FormControl size="small" sx={{ maxWidth: 300 }}>
        ...
        >
          {availableContentTypes.map((ct) => (
            <MenuItem key={ct} value={ct}>{CONTENT_TYPE_LABELS[ct] ?? ct}</MenuItem>
          ))}
        </Select>
      </FormControl>
      ...
      <FormControl size="small" sx={{ maxWidth: 300 }}>
        ...
        <Select
          ...
          onChange={(e) => setConfig({ ...config, duplicateStrategy: e.target.value as ImportConfig['duplicateStrategy'] })}
        >
      ...
      {selectedSource?.sourceType === 'api' && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField
            ...
            onChange={(e) => setConfig({ ...config, options: { ...config.options, query: e.target.value } })}
          ...
          <TextField
            ...
            onChange={(e) => setConfig({ ...config, options: { ...config.options, location: e.target.value } })}
          ...
          <TextField
            ...
            onChange={(e) => setConfig({ ...config, options: { ...config.options, limit: e.target.value } })}
            sx={{ maxWidth: 200 }}
          />
        </Box>
      )}
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, py: 4 }}>
      {isRunning && (
        <>
          ...
          <Typography variant="h6" sx={{ fontWeight: 600 }}>Importing...</Typography>
        </>
      )}
      {isDone && (
        <>
          <CheckCircle2 size={48} style={{ color: '#10b981' }} />
          <Typography variant="h6" sx={{ fontWeight: 600, color: '#10b981' }}>Import Complete</Typography>
        </>
      )}
      {isFailed && (
        <>
          <AlertCircle size={48} style={{ color: '#ef4444' }} />
          <Typography variant="h6" sx={{ fontWeight: 600, color: '#ef4444' }}>Import Failed</Typography>
        </>
      )}
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, width: '100%', maxWidth: 500 }}>
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
        ...
        <Typography variant="h5" sx={{ fontWeight: 700 }}>Import Wizard</Typography>
      </Box>
        ...
        {activeStep === 0 && (
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 4, gap: 1.5 }}>
            <Button
              ...
              startIcon={isStarting ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Play size={16} />}
              sx={{ textTransform: 'none' }}
            >
