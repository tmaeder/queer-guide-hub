...
export function SideBySideComparison({
  ...
  const fields = useMemo(
    () => getFieldsForEntity(entityType, { ...leftData, ...rightData }),
    [entityType, leftData, rightData],
  );
  ...
  const buildMergedData = (): Record<string, unknown> => {
    ...
    for (const field of fields) {
      merged[field.key] =
        choices[field.key] === 'left' ? leftData[field.key] : rightData[field.key];
    }
    return merged;
  };
    ...
    <div className="flex flex-col gap-4">
          ...
          {fields.map((field, i) => {
            ...
            return (
              <div
                ...
                style={{
                  ...gridCols,
                  backgroundColor: isDiff ? 'hsl(var(--foreground) / 0.55)' : 'transparent',
                }}
              >
                {/* Field Name */}
                ...
                <div
                  ...
                  style={{
                    ...
                    borderLeft:
                      choices[field.key] === 'left' && isDiff
                        ? '3px solid hsl(var(--muted-foreground))'
                        : '3px solid transparent',
                    backgroundColor:
                      choices[field.key] === 'left' && isDiff ? 'hsl(var(--muted))' : 'transparent',
                  }}
                ...
                <div
                  ...
                  style={{
                    cursor: isDiff ? 'pointer' : 'default',
                    borderLeft:
                      choices[field.key] === 'right' && isDiff
      ...
      {showActions && (
        <div className="flex gap-2 justify-end flex-wrap">
          ...
          {onKeepLeft && (
            <Button variant="outline" onClick={onKeepLeft} className="flex gap-1.5">
              <Check size={14} />
              Keep {leftLabel}
            </Button>
          )}
          {onKeepRight && (
            <Button variant="outline" onClick={onKeepRight} className="flex gap-1.5">
              <Check size={14} />
              Keep {rightLabel}
            </Button>
          )}
          {onMerge && leftId && rightId && (
            <Button
              onClick={handleMerge}
              style={{ backgroundColor: 'hsl(var(--muted-foreground))', color: 'white' }}
