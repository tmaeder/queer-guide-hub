...
const getStatusBadge = (status: string) => {
  switch (status) {
    case 'configured':
      return <Badge variant="default" sx={{ bgcolor: '#166534', color: 'white' }}>Configured</Badge>;
    case 'missing':
      return <Badge variant="destructive">Missing</Badge>;
    case 'error':
      return <Badge variant="secondary" sx={{ bgcolor: '#92400e', color: 'white' }}>Error</Badge>;
    default:
...
export const ApiKeysManager = () => {
  const { keys, requiredKeys, loading, createApiKey, updateApiKey, deleteApiKey, toggleApiKey, refreshKeys } = useApiKeys();
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editForm.service_name && editForm.service_name !== editingKey.service_name) updateData.service_name = editForm.service_name;
      if (editForm.key_name && editForm.key_name !== editingKey.key_name) updateData.key_name = editForm.key_name;
  ...
  const startEdit = (key: ApiKey) => {
    setEditingKey(key);
    setEditForm({ service_name: key.service_name, key_name: key.key_name, key_value: '', description: key.description || '', is_active: key.is_active });
  };
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            <DialogTrigger asChild>
              <Button><Plus style={{ height: 16, width: 16, marginRight: 8 }} />Add API Key</Button>
            </DialogTrigger>
            <DialogContent>
              <form onSubmit={handleCreateSubmit}>
                <DialogHeader>
                  <DialogTitle>Add New API Key</DialogTitle>
                  <DialogDescription>Store an API key securely for use in import operations</DialogDescription>
                </DialogHeader>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, py: 2 }}>
                  <div>
                    <Label htmlFor="service">Service</Label>
                    <Select value={createForm.service_name} onValueChange={(value) => setCreateForm(prev => ({ ...prev, service_name: value, key_name: '' }))}>
                  ...
                  {createForm.service_name && (
                    <div>
                      ...
                      <Select value={createForm.key_name} onValueChange={(value) => setCreateForm(prev => ({ ...prev, key_name: value }))}>
                  ...
                  <div>
                    ...
                    <Input id="key_value" type="password" value={createForm.key_value} onChange={(e) => setCreateForm(prev => ({ ...prev, key_value: e.target.value }))} placeholder="Enter the API key" required />
                  </div>
                  <div>
                    <Label htmlFor="description">Description (Optional)</Label>
                    <Textarea id="description" value={createForm.description} onChange={(e) => setCreateForm(prev => ({ ...prev, description: e.target.value }))} placeholder="Add a description" rows={3} />
                  </div>
                </Box>
                <DialogFooter>
                  <Button type="button" variant="outline" onClick={() => setShowCreateDialog(false)}>Cancel</Button>
                  <Button type="submit" disabled={!createForm.service_name || !createForm.key_name || !createForm.key_value}>Create API Key</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
      ...
      {loading ? (
        ...
        <>
          ...
          {requiredKeys.length > 0 && (
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr 1fr', md: 'repeat(3, 1fr)' }, gap: 2 }}>
              <Box sx={{ textAlign: 'center', p: 2, bgcolor: '#f0fdf4', borderRadius: 2 }}>
                <Typography variant="h5" sx={{ color: '#16a34a' }}>{configuredCount}</Typography>
                <Typography variant="body2" sx={{ color: '#16a34a' }}>Configured</Typography>
              </Box>
              <Box sx={{ textAlign: 'center', p: 2, bgcolor: '#fef2f2', borderRadius: 2 }}>
                <Typography variant="h5" sx={{ color: '#dc2626' }}>{missingCount}</Typography>
                <Typography variant="body2" sx={{ color: '#dc2626' }}>Missing</Typography>
              </Box>
              <Box sx={{ textAlign: 'center', p: 2, bgcolor: '#fffbeb', borderRadius: 2 }}>
                <Typography variant="h5" sx={{ color: '#d97706' }}>{errorCount}</Typography>
                <Typography variant="body2" sx={{ color: '#d97706' }}>Errors</Typography>
              </Box>
            </Box>
          )}
          ...
          {requiredKeys.length > 0 && (
            <Card>
              ...
              <CardContent>
                  ...
                  {requiredKeys.map((rk) => (
                    <Box
                      ...
                      sx={{
                        ...
                        borderRadius: 1,
                        border: '1px solid',
                        borderColor: rk.status === 'configured' ? 'rgba(34,197,94,0.3)' : rk.status === 'error' ? 'rgba(245,158,11,0.3)' : 'rgba(239,68,68,0.3)',
                        bgcolor: rk.status === 'configured' ? 'rgba(34,197,94,0.05)' : rk.status === 'error' ? 'rgba(245,158,11,0.05)' : 'rgba(239,68,68,0.05)',
                      }}
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flex: 1 }}>
                        {getStatusIcon(rk.status)}
                        <Box>
                          <Typography sx={{ fontWeight: 600, fontFamily: 'monospace', fontSize: '0.875rem' }}>
                            {rk.key_name}
                          </Typography>
                          {rk.used_by.length > 0 && (
                ...
                <Typography sx={{ mt: 2, fontSize: '0.75rem', color: 'text.secondary', fontStyle: 'italic' }}>
                  To add or update these keys, go to Supabase Dashboard &rarr; Project Settings &rarr; Edge Functions &rarr; Secrets
                </Typography>
              </CardContent>
            </Card>
          )}
            ...
            <CardContent>
              {keys.length === 0 ? (
                <Box sx={{ textAlign: 'center', py: 3 }}>
                  <Key style={{ height: 32, width: 32, margin: '0 auto 12px', color: 'var(--muted-foreground)' }} />
                  ...
                  {keys.map((key) => (
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Key style={{ width: 16, height: 16, color: key.is_active ? '#22c55e' : '#6b7280' }} />
                        <Box>
                          <Typography sx={{ fontWeight: 600, fontSize: '0.875rem' }}>{key.service_name} / {key.key_name}</Typography>
                          {key.description && <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>{key.description}</Typography>}
                          <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="outline" size="sm"><Trash2 style={{ height: 14, width: 14 }} /></Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader>
        ...
        <DialogContent>
          <form onSubmit={handleEditSubmit}>
            <DialogHeader>
              ...
              <DialogDescription>Update details. Leave key value empty to keep current key.</DialogDescription>
            </DialogHeader>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, py: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <div>
                ...
                <Input id="edit_service" value={editForm.service_name} onChange={(e) => setEditForm(prev => ({ ...prev, service_name: e.target.value }))} />
              </div>
              <div>
                <Label htmlFor="edit_key_name">Key Name</Label>
                <Input id="edit_key_name" value={editForm.key_name} onChange={(e) => setEditForm(prev => ({ ...prev, key_name: e.target.value }))} />
              </div>
              <div>
                <Label htmlFor="edit_key_value">New API Key Value (Optional)</Label>
                <Input id="edit_key_value" type="password" value={editForm.key_value} onChange={(e) => setEditForm(prev => ({ ...prev, key_value: e.target.value }))} placeholder="Leave empty to keep current key" />
              </div>
              <div>
                <Label htmlFor="edit_description">Description</Label>
                <Textarea id="edit_description" value={editForm.description} onChange={(e) => setEditForm(prev => ({ ...prev, description: e.target.value }))} rows={3} />
              </div>
            </Box>
            <DialogFooter>
