  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-2.5 pt-1">
        <span className="text-title font-bold tabular-nums">{nf.format(d.count)}</span>
        {(d.orphanCount != null && d.orphanCount > 0) || d.dupCount > 0 ? (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {d.orphanCount != null && d.orphanCount > 0 && (
              <Badge
                variant="destructive"
                className="text-2xs px-1 py-0 tabular-nums"
                title="Records missing their primary link"
              >
            ...
            {d.dupCount > 0 && (
              <Badge
                variant="secondary"
                className="text-2xs px-1 py-0 tabular-nums"
                title="Records flagged as duplicates"
              >
