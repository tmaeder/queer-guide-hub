      ...
      >
        {items.map((card, i) => (
          <a
            key={i}
            href={card.href ?? '#'}
            className="snap-start shrink-0 w-64 h-80 md:w-80 md:h-96 relative rounded-container overflow-hidden group ring-1 ring-border/60 shadow-[var(--shadow-aceternity-sm)]"
          >
            <motion.img
              src={card.src}
              alt={card.title}
