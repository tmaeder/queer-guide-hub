...
import { Search, Filter, TrendingUp, BarChart3, Image, Network, Tag, LayoutGrid, List, SortAsc, SortDesc } from "lucide-react";
    ...
    <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 }, mb: 3, bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, alignItems: 'center' }}>
        ...
        <Box sx={{ ml: 'auto', display: 'flex', gap: 1, alignItems: 'center' }}>
          <Select value={sortBy} onValueChange={(value: string) => onSortByChange(value as SortOption)}>
      ...
      {(filterCategory !== 'all' || usageFilter !== 'all' || hasImageFilter) && (
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 2, alignItems: 'center' }}>
          ...
          <Box
            onClick={() => { onFilterCategoryChange('all'); onUsageFilterChange('all'); onHasImageFilterChange(false); }}
