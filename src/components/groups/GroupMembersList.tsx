...
interface GroupMember {
  ...
  profiles: { display_name: string; avatar_url: string; social_links?: Record<string, unknown>; };
}
interface GroupMembersListProps {
  members: GroupMember[]; canManage: boolean; groupId?: string;
  onStartConversation?: (userId: string) => void; onMembersChanged?: () => void;
}
...
export function GroupMembersList({ members, canManage, groupId, onStartConversation, onMembersChanged }: GroupMembersListProps) {
  ...
  const handleRemove = async (member: GroupMember) => {
    ...
    onMembersChanged?.(); setRemovingId(null);
  };
  return (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                {member.profiles.social_links && Object.keys(member.profiles.social_links).length > 0 && (
                  <div className="mt-2"><SocialLinksDisplay socialLinks={member.profiles.social_links} size="sm" /></div>
                )}
              </div>
            </div>
            <div className="flex items-center gap-2">
      ...
      <AlertDialog open={!!removingId} onOpenChange={(open) => !open && setRemovingId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Remove Member</AlertDialogTitle>
            <AlertDialogDescription>Remove this member from the group? They can rejoin later.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { const m = members.find(x => x.user_id === removingId); if (m) handleRemove(m); }}>Remove</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
