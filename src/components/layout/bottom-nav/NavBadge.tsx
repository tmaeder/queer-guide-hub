...
export function NavBadge({ count, dot, label }: NavBadgeProps) {
  if (dot) {
    return (
      <span
        ...
        className="absolute -end-0.5 -top-0.5 h-2 w-2 rounded-full bg-background"
      />
    );
  }
  ...
  return (
    <span
      aria-label={label}
      className="absolute -end-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-background px-1 text-2xs font-medium leading-none text-foreground"
    >
      {count > 99 ? '99+' : count}
    </span>
  );
