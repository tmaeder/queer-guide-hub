...
/** Trust-tier ladder + progress to next tier. Moved from ProfileTiers; static (no motion). */
export function TrustTierLadder() {
  const { data, isLoading } = useMyTier();

  if (isLoading || !data) {
    return <div className="h-8 w-40 animate-pulse rounded-element bg-muted" />;
  }
              ...
              {!isLast && (
                <span
                  ...
                  className={
                    ...
                    (reached && TIER_ORDER.indexOf(tier) > i ? 'bg-foreground' : 'bg-border')
                  }
                />
              )}
              <span
