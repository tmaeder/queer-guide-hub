...
const WeeklyEventsSlider = React.memo(() => {
  ...
  return <section style={{ backgroundColor: 'rgba(var(--muted-rgb), 0.1)', padding: isMobile ? '32px 16px' : '64px 16px' }}>
      <Box sx={{ maxWidth: 'lg', mx: 'auto' }}>
                ...
                <Card style={{ transition: 'all 0.3s', height: '100%' }}>
                  <CardContent style={{ padding: 24, height: '100%', display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 2 }}>
                      ...
                      {event.is_free ? <Badge variant="outline" style={{ fontSize: '0.75rem' }}>
                          Free
                        </Badge> : event.price_min && <Badge variant="outline" style={{ fontSize: '0.75rem' }}>
                          ${event.price_min}+
                        </Badge>}
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 2, flexGrow: 1 }}>
                      ...
                      {(event.city || event.venue_name) && <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'text.secondary' }}>
                          <MapPin style={{ height: 16, width: 16, flexShrink: 0 }} />
                          <Box component="span" sx={{ fontSize: '0.875rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {event.venue_name || event.city}
                          </Box>
                        </Box>}
                    </Box>

                    {event.description && <Typography sx={{ fontSize: '0.875rem', color: 'text.secondary', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', mb: 2 }}>
                        {event.description}
                      </Typography>}
        ...
        {isMobile && weeklyEvents.length > 1 && <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
            <Box sx={{ display: 'flex', gap: 1 }}>
              {weeklyEvents.slice(0, 5).map((_, index) => <Box key={index} sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: 'action.hover' }} />)}
            </Box>
          </Box>}
      </Box>
    </section>;
