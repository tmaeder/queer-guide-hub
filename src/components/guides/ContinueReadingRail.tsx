...
/**
 * "Continue reading" rail — signed-in users only. Shows guides (any format)
 * with an in-progress guide_reads row. Renders nothing for anonymous users
 * or when nothing is in progress.
 */
  ...
  return (
    <section className="my-8" aria-labelledby="continue-reading-heading">
      <header className="mb-4">
        ...
        >
          {t('guides.continueReading.title', 'Continue reading')}
        </h2>
      </header>
          ...
          return (
            <li key={it.guide_id}>
              <LocalizedLink
                ...
                className="group flex gap-4 items-start rounded-element p-4 no-underline hover:bg-muted/40"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-2xs uppercase tracking-wide text-muted-foreground mt-1">
                    {t('guides.continueReading.pctRead', '{{pct}}% read').replace(
                      '{{pct}}',
                      String(it.scroll_pct),
                    )}
                  </p>
                </div>
              </LocalizedLink>
