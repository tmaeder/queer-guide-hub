...
import { LAYER_COLORS, MONO_HEAT_STOPS, type LayerType } from '@/hooks/useExploreMapData';
    ...
    <AnimatePresence initial={false} mode="wait">
      {!open ? (
        <motion.button
          ...
          className={`absolute ${bottomClass} left-3 z-10 inline-flex items-center gap-1.5 rounded-element bg-background/95 px-4 py-1.5 text-13 text-foreground backdrop-blur-md hover:bg-background focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`}
        >
        ...
        <motion.div
          ...
          className={`absolute ${bottomClass} left-3 z-10 w-56 rounded-container bg-background/95 p-4 backdrop-blur-md`}
        >
          ...
          {areaLayers.length > 0 && (
            <div className={showPins && pointLayers.length > 0 ? 'mt-2 pt-2' : ''}>
