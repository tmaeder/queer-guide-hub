...
/**
 * Guards 99960101100100 — the 2026-08-30 flip residue: a misfiled summary on
...
const MIGRATION = join(
  process.cwd(),
  'supabase/migrations/99960101100100_tag_prose_surname_stubs_and_queening.sql',
);
...
describe('99960101100100 — surname stubs and the queening summary', () => {
  ...
  it('declares an actor — load-bearing for queening, which is human_reviewed', () => {
    ...
    expect(statements).toMatch(
      /set_config\(\s*'app\.actor'\s*,\s*'migration:99960101100100[^']*'\s*,\s*true\s*\)/i,
    );
  });
