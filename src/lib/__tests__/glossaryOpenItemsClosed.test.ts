...
const FACET = '51700101100000_hiv_aids_topic_facet.sql';
const SWEEP = '51700101100100_styleguide_drift_vibrant_sweep.sql';
...
describe('51700101100000 — hiv-aids stays a facet', () => {
  const apply = applyBlockOf(FACET);
  const verify = verifyBlockOf(FACET);

  it('declares an actor', () => {
    expect(apply).toContain(
      "set_config('app.actor', 'migration:51700101100000_hiv_aids_topic_facet', true)",
    );
  });
...
describe('51700101100100 — vibrant sweep', () => {
  const apply = applyBlockOf(SWEEP);
  const verify = verifyBlockOf(SWEEP);

  it('declares an actor', () => {
    expect(apply).toContain(
      "set_config('app.actor', 'migration:51700101100100_styleguide_drift_vibrant_sweep', true)",
    );
  });
