 ...
 *
 *  The filter must match a DEFINITION, not a mention. It used to match any file
 *  containing the string `enforce_tag_wikidata_identity`, which meant that the
 *  moment any later migration merely NAMED the trigger — in a comment explaining
 *  why it behaves the way it does, say — `.sort().pop()` picked that file
 *  instead, `triggerFn` sliced to empty, and all ten assertions failed at once
 *  while the actual seal was untouched. Requiring `function public.<name>` keeps
 *  the selector pointed at the file that carries the body. */
function sealSql(): string {
  const file = readdirSync(DIR)
    .filter((f) => /^\d{14}_.*\.sql$/.test(f))
    .filter((f) =>
      readFileSync(join(DIR, f), 'utf8').includes('function public.enforce_tag_wikidata_identity'),
    )
    .sort()
    .pop();
