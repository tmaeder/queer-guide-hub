...
/**
 * The second shadow-alias pass (`20261219100000`) and the seal that stops it
 * needing a third (`20261219100100`).
 *
...
const pass = readFileSync(join(DIR, '20261219100000_tag_shadow_alias_pass_2.sql'), 'utf8');
const seal = readFileSync(join(DIR, '20261219100100_tag_shadow_seal.sql'), 'utf8');
...
describe('tag shadow alias pass 2', () => {
  ...
  it('refuses to seal a corpus that still violates the invariant', () => {
    expect(seal).toMatch(/20261219100000 must apply first/);
  });
});
