/**
 * extractFileContent — heavy file-extraction pipeline for flyer scan.
 *
 * Kept SEPARATE from `fileExtractors.ts` (which holds the lightweight
 * `isAcceptedFile` / size constants used eagerly by the upload UI). This
 * module lazy-imports `pdfjs-dist` and `mammoth`; isolating it ensures those
 * heavy chunks never leak onto the critical path of pages that only need the
 * light accept-check. Import this only from the flyer-scan flow.
 */

import { UploadErrorException, makeUploadError } from './uploadErrors';
import { MAX_FILE_SIZE_BYTES, type ExtractedContent } from './fileExtractors';
