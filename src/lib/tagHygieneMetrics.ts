...
export const HYGIENE_METRICS: HygieneMetric[] = [
  ...
  },
  {
    ...
    label: 'Facet published as a page',
    zero: true,
    hint: 'A marketplace attribute facet belongs to no glossary category by decision, so it must never publish /tags/:slug. `enforce_tag_facet_page_gate` deindexes one at write time, which makes this a write-time invariant rather than a queue depth — non-zero means the trigger was bypassed or dropped.',
  },
