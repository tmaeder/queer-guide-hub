...
export type EntityKind = 'person' | 'venue' | 'event' | 'glossary_term' | 'unknown';
...
export function classifyEntity(rawInput: ClassifyInput): ClassifyResult {
  ...
  if (text.length > 0) {
    for (const re of PERSON_PHRASE_PATTERNS) {
      if (re.test(textLc)) {
        bumpSignal(scores, 'person', 2, signals, `phrase:${re.source.slice(0, 30)}`);
        break;
      }
    }
  }
  ...
  if (
    /^[A-Z][\p{L}'’-]+(?:\s+[A-Z][\p{L}'’.-]+){1,3}$/u.test(name) &&
    !PLACE_KEYWORDS.some((k) => nameLc.includes(k))
  ) {
  ...
  if (asString(input.accommodation_type))
    bumpSignal(scores, 'venue', 4, signals, 'has_accommodation_type');
  ...
  if (text.length > 0) {
    for (const re of VENUE_PHRASE_PATTERNS) {
      if (re.test(textLc)) {
        bumpSignal(scores, 'venue', 2, signals, `phrase:${re.source.slice(0, 30)}`);
        break;
      }
    }
  }
  ...
  if (text.length > 0) {
    for (const re of EVENT_PHRASE_PATTERNS) {
      if (re.test(textLc)) {
        bumpSignal(scores, 'event', 1, signals, `phrase:${re.source.slice(0, 30)}`);
        break;
      }
    }
  }
  ...
  if (
    wordCount > 0 &&
    wordCount <= 4 &&
    (isAllLower || isShortAcronym) &&
    !hasPhysicalLocation &&
    !asString(input.birth_date) &&
    !asString(input.death_date) &&
    !qid
  ) {
  ...
  if (description) {
    for (const re of GLOSSARY_PHRASE_PATTERNS) {
      if (re.test(descLc)) {
        bumpSignal(scores, 'glossary_term', 4, signals, `phrase:${re.source.slice(0, 30)}`);
        break;
      }
    }
  }
 ...
 */
export function expectedKindForTargetTable(
