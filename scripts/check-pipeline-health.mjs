...
//     silent while the backlog compounds.
{
  ...
  if (!res.ok) {
    ...
    console.warn(`⚠ dedup_signals_all → HTTP ${res.status} (RPC missing? migration 20810101100500)`)
    console.warn('  This check measured NOTHING — it did not pass.')
  } else {
    ...
    for (const t of types) {
      ...
      if (unrev > 0) {
        ...
        console.error(`  The live merge core has drifted from 20810101100000/100200 — those merges cannot be undone.`)
        FAILED = true
      }
