...
// surface (same reason news_image_signals and venue_dup_signals are separate).
{
  ...
  if (!res.ok) {
    ...
    console.warn(`⚠ analytics_hygiene_stats → HTTP ${res.status} (20520301100500 not applied?)`)
    console.warn('  This check measured NOTHING — it did not pass.')
  } else {
    ...
    if (scheduled < 2) {
      ...
      console.error('  Expected umami_retention and user_events_retention (migration 20520301100100).')
      FAILED = true; sectionOk = false
    }
