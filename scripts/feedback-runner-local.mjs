 ...
 *
 * Autonomous runs (auto_dispatched=true, queued by feedback-auto-dispatch) also
 * self-test: after the fix PR is opened the daemon enqueues the retest matrix
 * (auto_enqueue_retests), runs it, and once all retests are green merges the PR
 * (gh pr merge --squash) and resolves the story via record_merge. The
 * feedback_auto_merge kill switch + daily cap (admin_automations) gate the merge.
 * Manual local dispatches are untouched — they still wait for human retest/merge.
 *
...
async function handleRoutineRun(run) {
  ...
  try {
    ...
    // Autonomous runs self-test: queue the retest matrix immediately so the
    // retest poll picks it up (manual local dispatches are tested from the UI).
    if (run.auto_dispatched) {
      const { error: enqErr } = await supa.rpc('auto_enqueue_retests', { p_run_id: run.id });
      if (enqErr) log(`auto_enqueue_retests failed for ${run.id}:`, enqErr.message);
      else log(`queued retests for autonomous run ${run.id}`);
    }
  } catch (e) {
...
async function handleMerge(candidate) {
  const { run_id: runId, pr_url: prUrl, story_id: storyId } = candidate;
  log(`auto-merging run ${runId} (story ${storyId}) → ${prUrl}`);

  if (DRY_RUN) {
    log(`[dry-run] would gh pr merge --squash ${prUrl}`);
    await supa.rpc('record_merge', { p_run_id: runId, p_merge_sha: 'dry-run-merge-sha' });
    return;
  }
  ...
  if (merge.status !== 0) {
    // Branch protection / pending checks / conflicts. Leave the run as
    // fix_proposed; it stays visible for manual review and will be retried.
    log(`gh pr merge failed for ${runId} (left for review): ${merge.stderr || merge.stdout}`);
    return;
  }
  ...
  if (view.status === 0) mergeSha = view.stdout.trim() || null;

  const { error } = await supa.rpc('record_merge', { p_run_id: runId, p_merge_sha: mergeSha });
  if (error) log(`record_merge failed for ${runId}:`, error.message);
  else log(`merged + resolved story ${storyId} (sha ${mergeSha ?? 'unknown'})`);
}
...
async function tick() {
  ...
  const { data: runs, error: runErr } = await supa
    .from('feedback_routine_runs')
    .select('id,story_id,prompt,external_ref,status,runner,auto_dispatched')
    .eq('runner', 'local')
  ...
  // 3) autonomous fix PRs whose retests are all green → merge + resolve.
  //    The RPC enforces the feedback_auto_merge kill switch + daily cap.
  const { data: mergeable, error: mergeErr } = await supa.rpc('routine_runs_ready_to_merge', {
    p_limit: 1,
  });
  if (mergeErr) {
    log('poll merge error:', mergeErr.message);
  } else if (mergeable && mergeable.length > 0) {
    await handleMerge(mergeable[0]);
  }
}
