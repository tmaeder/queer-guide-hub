...
// 20261021110000 and 20270301154207. If this appears in the logs, re-measure the
...
if (!res.ok && body?.includes('57014')) {
  ...
  console.warn(
    `⚠ retry ${res.ok ? 'SUCCEEDED' : 'FAILED'} after ${Date.now() - t0}ms. The RPC is near its 8s ` +
      'ceiling — re-measure per arm (see migration 20270301154207) rather than retrying harder.',
  )
}
