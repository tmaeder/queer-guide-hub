...
export default tseslint.config(
  ...
  },
  {
    ...
    plugins: {
      ...
      queerguide: {
        rules: {
          'no-supabase-from-in-pages': noSupabaseFromInPages,
          ...
          paths: [
            {
              name: '@mui/material',
              message:
                'MUI migration complete (ADR 0001). Use shadcn/ui components + Tailwind. See docs/mui-to-shadcn-migration-recipe.md.',
            },
            {
              name: '@mui/icons-material',
              message: 'MUI migration complete (ADR 0001). Use lucide-react icons instead.',
            },
            {
              name: '@mui/lab',
              message: 'MUI migration complete (ADR 0001). Use shadcn/ui components + Tailwind.',
            },
          ],
          patterns: [
            {
  ...
  // See docs/design-system/README.md and CLAUDE.md design section.
  {
    ...
    ignores: [
      ...
      // Drug-interaction risk scale (dangerous → low-risk-synergy) — the same
      // class of exception as the trip-safety traffic light above, and the
      // single home of its locked channel triples. Listed even though the file
      // currently passes (it stores HSL channel triples, not hex/rgb/hsl
      // literals) so the exception is registered here rather than depending on
    ...
    rules: {
      'no-restricted-syntax': [
        ...
        {
          selector:
            'Literal[value=/\\bduration-(75|100|150|200|300|500|700|1000)\\b|\\bduration-\\[/]',
          message:
        ...
        {
          selector: 'Literal[value=/^#[0-9a-fA-F]{3,8}$|^rgba?\\(\\s*\\d|^hsla?\\(\\s*\\d/]',
          message:
            'Hardcoded color literal — use design tokens (hsl(var(--foreground)), hsl(var(--muted)), etc.).',
        },
        {
          selector: 'Literal[value=/shadow-\\[var\\(--shadow-(aceternity|glow)|shadow-aceternity/]',
          message:
  ...
  {
    files: ['src/**/*.{ts,tsx}'],
    ignores: ['src/integrations/supabase/types.ts', 'src/**/__tests__/**', 'src/test/**'],
    rules: {
      'no-restricted-syntax': [
        'warn',
        {
        ...
        {
          selector: 'Literal[value=/\\brounded-(xs|sm|md|lg|xl|2xl|3xl|4xl)\\b/]',
          message:
        ...
        {
          selector: 'Literal[value=/(\\srounded(?![-\\w]))|(^rounded\\s)/]',
          message:
  ...
  // cannot see. Admin tree uses StatusBadge / semantic tokens only.
  {
    ...
    rules: {
      'no-restricted-syntax': [
        ...
        {
          selector: 'Literal[value=/\\bshadow-(md|lg|xl|2xl)\\b/]',
          message: 'Shadows are disabled (CLAUDE.md § Design). Use border or bg-muted for depth.',
        },
        {
          selector: 'Literal[value=/^(?!.*\\bfrom-(black|background)\\b).*\\bbg-gradient-to-/]',
          message:
  ...
  // any remaining instances are flushed.
  {
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        ...
        {
          selector: 'Literal[value=/(gradient\\(|var\\([^)]*,\\s*)[^)]*#[0-9a-fA-F]{3,8}/]',
          message:
        ...
        {
          selector: 'Literal[value=/\\bfont-extrabold\\b/]',
          message: 'Weight scale is 400/500/600/700. Use font-bold (700).',
        },
        {
          selector: 'Literal[value=/\\brounded-(xs|sm|md|lg|xl|2xl|3xl|4xl)\\b/]',
          message:
      ...
      'no-restricted-imports': [
        'error',
        {
          paths: [
            {
              name: 'framer-motion',
              message: 'Admin tree is motion-free (Cluster 3). Remove decorative motion.',
            },
            {
              name: 'motion/react',
              message: 'Admin tree is motion-free (Cluster 3). Remove decorative motion.',
          ...
          patterns: [
            {
              group: ['@/components/effects/*'],
              message:
                'Aceternity effect components are not allowed in the admin tree (Cluster 3).',
            },
            {
              group: ['@/components/animation/*'],
              message:
                'Decorative animation wrappers are not allowed in the admin tree (Cluster 3).',
            },
            {
              group: ['@/components/motion/*'],
              message: 'Decorative motion wrappers are not allowed in the admin tree (Cluster 3).',
            },
            {
              group: ['@mui/*', '@mui/**'],
              message: 'MUI migration complete (ADR 0001). Use shadcn/ui components + Tailwind.',
  ...
  // Page frames come from <PageContainer>, never a hand-rolled
  // `container mx-auto px-4 py-8`. See docs/design-system/README.md §Page layout.
  //
  // Scoped to src/pages because a bar, a banner or a footer legitimately applies
  // the same ladder to an element it owns (Header, Footer, CookieConsentBanner)
  // — those are chrome spanning the viewport, not page frames.
  //
  // A NAMED rule on purpose: as another no-restricted-syntax selector it would
  // have to be re-stated in all four blocks that match src/pages/**, and one
  // miss silently disables selectors that are already load-bearing (flat config
  // replaces that rule WHOLESALE per file — precedent #2049). A block adding
  // only a distinctly-named rule merges instead of replacing.
  {
