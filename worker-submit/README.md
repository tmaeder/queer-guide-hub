...
- `POST /submit` — body: `SubmitBody`, header: `Authorization: Bearer <supabase JWT>`
  ...
  - Hashes payload (matching [staging-publisher.ts](../Dev/web/scraper/src/db/staging-publisher.ts) `stableStringify`).
...
# migrate the DB (use Supabase CLI or MCP — Dev/web/supabase is gitignored)
