...
- **Source:** [src/components/ui/select.tsx:135](Dev/web/src/components/ui/select.tsx) extracts `aria-label` / `aria-labelledby` / `aria-describedby` via `SELECT_ARIA_KEYS` and forwards to `inputProps`, but call sites on filter pages don't pass them. `displayEmpty` + placeholder is the only label.
...
- **Source:** i18next init in `Dev/web/src/i18n` and routing in `Dev/web/src/App.tsx` (`LocaleRouter`). No `document.documentElement.setAttribute('lang'/'dir', …)` on locale change.
...
- **Source:** an `<MuiCircularProgress role="progressbar">` rendered without `aria-label`, likely a stats-strip skeleton. Many `<CircularProgress />` usages across the codebase have no label (samples: [src/components/loading/OptimizedLoader.tsx:123](Dev/web/src/components/loading/OptimizedLoader.tsx:123), [src/components/ui/country-autocomplete.tsx:140](Dev/web/src/components/ui/country-autocomplete.tsx:140), [src/components/admin/AffiliatePartnersManager.tsx:101](Dev/web/src/components/admin/AffiliatePartnersManager.tsx:101)).
...
- **Fix:** address at the token layer. Audit `--muted-foreground`, `text.secondary`, and the brand link tint in [src/index.css](Dev/web/src/index.css) and [src/theme/muiTheme.ts](Dev/web/src/theme/muiTheme.ts). Apply per-context where token can't be raised without breaking the design.

### C6 · Admin pipeline-builder forms have orphan `<label>` elements
- **WCAG:** 1.3.1 / 3.3.2 / 4.1.2
- **Source:** ESLint flags 9 violations in [AccessDialog.tsx:125,135](Dev/web/src/components/admin/pipeline-builder/panels/AccessDialog.tsx), [TemplateLibrary.tsx:222,231,240](Dev/web/src/components/admin/pipeline-builder/panels/TemplateLibrary.tsx), [IntegrationsTab.tsx:138,147,158,167](Dev/web/src/components/admin/pipeline-builder/tabs/IntegrationsTab.tsx).
...
- **Source:** [CommentNode.tsx:94](Dev/web/src/components/admin/pipeline-builder/nodes/CommentNode.tsx:94), [GroupNode.tsx:81](Dev/web/src/components/admin/pipeline-builder/nodes/GroupNode.tsx:81), [NewsCard.tsx:190](Dev/web/src/components/news/NewsCard.tsx:190).
...
- **Source:** [Header.tsx](Dev/web/src/components/layout/Header.tsx) — `a11y-header.spec.ts` tests the hamburger drawer but not the desktop dropdown menus.
...
- **Source:** [AdminPipelines.tsx](Dev/web/src/pages/AdminPipelines.tsx) + the visual graph editor in `components/admin/pipeline-builder/`.
...
- [src/components/ui/carousel.tsx](Dev/web/src/components/ui/carousel.tsx) — verify auto-advance respects `prefers-reduced-motion` and pauses on focus/hover.
...
- **R6** — Promote 16 `jsx-a11y` rules from `warn` to `error` in [eslint.config.js:41-57](Dev/web/eslint.config.js) once C6 is fixed (lint baseline at zero).
