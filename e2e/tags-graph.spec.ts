...
test.describe('@p0-1 /tags graph view', () => {
  test('graph view loads tags + links from get_tag_graph_data (no 403)', async ({ page }) => {
    ...
    // The graph is a display MODE now, reachable by URL — it used to be
    // component state, so it could not be linked to and Back could not leave
    // it. Entering directly is the primary path; the tab click is covered by
    // tags-smoke.
    await page.goto('/tags?view=graph');
